...
Page({
  ...
  onLoad: function (options) {
    let page = this;
    wx.removeStorageSync('isShare') //如果是分享进入，清楚分享标识
  },
  onShow: function () {
    let page = this;
    ...
    app.checkInfo(function () {
      try {
        ...
        getReceiptList(page, wxBizKey, refreshFlag)
      }
      catch (e) {
      }
    })
...
// 请求列表数据
function getReceiptList(page, wxBizKey, refreshFlag, cb) {
  page.setData({
    loadingHidden: false
  })
  wx.request({
    url: basePath + 'receivebox/index',
    data: {
      wxBizKey: wxBizKey,
      cur_page: cur_page + 1
    },
