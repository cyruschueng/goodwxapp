...
Page({
  ...
  go_user_order_bind: function (e) {
    wx.navigateTo({
      url: '/pages/restaurant/restaurant-order-list/index?dish_id=' + this.data.this_dish_id
    });
  },
  ...
  call_phone_bind: function () {
    var that = this;
    wx.makePhoneCall({
      phoneNumber: that.data.dish_data.dish_info[0].dish_con_phone
    });
  },
  //跳转选址页面
        ...
        if (res.confirm == true) {
          ...
          wx.request({
            ...
            success: function (res) {
              ...
              // console.log(that.data.this_dish_id);
              if (res.data.isSuccess == "1") {
                that.initCartData();
  ...
  get_location_bind:function(e){
    ...
    wx.openLocation({
      ...
      address: that.data.dish_data.dish_info[0].address
    });
