...
Page({
      ...
      success: function(res) {
        ...
        wx.request({
          ...
          data: {
            wxappid: con.wxappid,
            fansid: res.data,
            cid: that.data.this_dish_id
          },
          header: {
            "content-type": "application/x-www-form-urlencoded"
          },
          success: function (res) {
            console.log(res.data, 333666);
            var boxPrice_single = res.data.info.box_price;
            var all_g_number = res.data.info.all_g_number;
            var boxprice = Number(all_g_number) * boxPrice_single;
            var yunprice = res.data.info.freight;
            var caiPrice = res.data.info.all_g_price;
            var totalPrice = Number(boxprice)+ Number(yunprice)+Number(caiPrice);
            ...
            that.setData({
              ...
              yunPrice: res.data.info.freight,
              boxPrice_single: boxPrice_single,
              boxPrice: boxprice,
  ...
  order_formSubmit:function(e){
    ...
    if (that.data.wx_address_info == null){
      ...
      orderInfo.totalprice = that.data.totalPrice;
                  ...
                  if (res.errMsg == 'requestPayment:ok') {
                    ...
                    wx.showModal({
                      ...
                      success: function (res) {
                        console.log(res);
                        wx.redirectTo({
                          url: '../restaurant-order-info/info?ordersn=' + that.data.ordersn + "&dish_id=" + that.data.this_dish_id
                        });
                      }
                    });
                  }
                  ...
                  //if (res.errMsg == 'requestPayment:fail cancel') {         
                    ...
                    wx.showModal({
                      ...
                      success: function (res) {
                        console.log(res);
                        wx.redirectTo({
                          url: '../restaurant-order-list/index?ordersn=' + that.data.ordersn + "&dish_id=" + that.data.this_dish_id 
