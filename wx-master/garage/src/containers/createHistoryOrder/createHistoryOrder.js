...
Page(Object.assign({
  ...
  submitStep1() {
    ...
    this.submit(data, function(res) {
      if (res.resultCode == '1' && res.resultObject.id) {
        wx.removeStorageSync('historyFormStep1')
        var step=3;
  ...
  skipStep3(){
    var data={orderStep:3,skipCheckCar:1,saveType:1,status:7,id:this.data.id},
        self=this;
    this.submit(data, function(res) {
      if (res.resultCode == '1') {
        self.setData({step: 4,skipStep3:true})
      }
    })
  },
  submitStep4(e,saveType) {
    ...
    this.submit(data, function(res) {
      if (res.resultCode == '1' && data.saveType=='1') {
        wx.redirectTo({
          url: '/src/containers/orderDetail/orderDetail?id=' + data.id
        })
      }
    })
  },
  ...
  emptyJudgement(key) {
    ...
    if (key === 'form_step1'&&this.data.status=='1') {
      wx.setStorageSync('historyFormStep1', JSON.stringify(this.data.form_step1))
    }
  },
      ...
      ajax({
        ...
        success: function(res) {
          ...
          if(_step){
            ...
            var step=orderStep=='1'?3:orderStep=='3'?4:1
          }
  ...
  onLoad: function(options) {
    ...
    if (options.id) {
      ...
      form1=JSON.parse(wx.getStorageSync('historyFormStep1')||"{}");
      if(form1.sex==2){
         this.setData({
            gender:[{name: '1',value: '先生',checked: false}, {name: '2',value: '女士',checked: true}]
         })
  ...
  _queryReceiveCarUserInfo(licence){
     ...
     ajax({
         ...
         success:function(res){
              console.log(res)
              if(res.resultCode==1&&res.resultObject){
                let {
                  autoModelChnName,
                  sex,
                  userName,
  ...
  handleCarSeriesChoosed(e){
      var id=e.currentTarget.dataset.id;
      console.log(this.data.carModel+id)
