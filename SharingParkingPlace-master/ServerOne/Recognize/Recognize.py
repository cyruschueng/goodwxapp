...
class Recognize(object):
    def __init__(self):
        self.__index = {u"京": 0, u"沪": 1, u"津": 2, u"渝": 3, u"冀": 4, u"晋": 5, u"蒙": 6, u"辽": 7, u"吉": 8, u"黑": 9, u"苏": 10, u"浙": 11, u"皖": 12,
        ...
        self.__chars = ["京", "沪", "津", "渝", "冀", "晋", "蒙", "辽", "吉", "黑", "苏", "浙", "皖", "闽", "赣", "鲁", "豫", "鄂", "湘", "粤", "桂",
    ...
    def __load(self):
        self.__model  = self.__Getmodel_tensorflow(65)
        #网络
        self.__model_ch = self.__Getmodel_ch(31)

        self.__model_ch.load_weights("./Model/char_chi_sim.h5")
        self.__model.load_weights("./Model/char_rec.h5")
    ...
    def __Getmodel_ch(self,nb_classes):
        # nb_classes = len(charset)
    ...
    def SimplePredict(self,image,pos):
        ...
        if pos!=0:
            res = np.array(self.__model.predict(np.array([image]))[0])
        else:
            res = np.array(self.__model_ch.predict(np.array([image]))[0])
        ...
        return self.__chars[max_id+zero_add]

if __name__=="__main__":
    test=Recognize()
    img=cv2.imreaad("./Cut/1.jpg",0)
    print(test.SimplePredict(img,0))
