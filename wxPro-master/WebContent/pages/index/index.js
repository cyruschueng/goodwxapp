...
Page({
  ...
  bindTypeTap: function (e) {
    //参数名会自动转小写且不允许下划线
    wx.navigateTo({
      url: '/pages/types/types?blogType=' + (e.currentTarget.dataset.blogtype ? e.currentTarget.dataset.blogtype : '')
    })
  },
  ...
  init: function (blogType, lastBlogId) {
    app.showLoading()
    isLoading = true
    ...
    if (!this.data.hotBlogs || this.data.hotBlogs.length < 1) {
      wx.request({
        url: app.globalData.baseURL + 'blog/hotBlog.action',
        data: {
  ...
  onReachBottom: function () {
    if (!isLoading) {
      if (this.data.blogs && (this.data.blogs.length % 6) == 0) {
        this.init(this.data.blogType, this.data.blogs[this.data.blogs.length - 1].blogId)
      }
    }
  },
