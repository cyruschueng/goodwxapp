...
const match = { //匹配对象：包含数据和函数
  ...
  init() {
    ...
    const loopMatch = setInterval(() => {
      if (finished) {
        finished = false
        let testtunnelId = players[this.queueData[0]] ? players[this.queueData[0]].tunnelId : null;
        tools.broadcast([testtunnelId], 'test', {test:1})
        for (let index1 = 0; index1 < this.queueData.length; index1++) {
          ...
          if (player1.friendsFightingRoom === undefined || player1.friendsFightingRoom === null) {
            for (let index2 = index1; index2 < this.queueData.length; index2++) {
              ...
              if ((player2.friendsFightingRoom === undefined || player2.friendsFightingRoom === null) && player2.sortId === player1.sortId && Math.abs(player2.score - player1.score) < option.MAX_SCORE_GAP && player2.openId !== player1.openId) {
...
console.log('match_success++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++');
  ...
  createRoom(openId1, openId2) {
    ...
    //library,默认包含5道题目
    mysql('question_detail').where((players[openId1].sortId == 1) ? {} : {
      sort_id: players[openId1].sortId
    }).select('*').orderByRaw('RAND()').limit(option.QUESTION_NUMBER).then(res => {
  ...
  async storeFightingRecord(openId, fightingResult, runAway = false) {
    ...
    try {
      ...
      if (myRecord.openId_winner && myRecord.openId_loser) {
        ...
        try {
          await mysql('fighting_record').insert({
            id: null,
            room_name,
            run_away,
            open_id_winner,
...
module.exports = {
  get: async ctx => { //响应用户开始进行websocket连接，信道服务器连接成功后通知客户端
    ...
    if (openId in players) { //如果已经存在openId,则说明只需更新信道ID
      ...
      let score = await mysql('cSessionInfo').where({
        open_id: data.userinfo.openId
      }).select('score') //[{score:4324}]
