	...
	*/
	...
	Img_scroll.prototype = {
		...
		var str ='',_this = this;
			...
			var srcs = this.config.data;
			var i,len=srcs.length,COUNT=0;
			for(i = 0;i<len;i++){
				var _src = srcs[i].src;
				var img = new Image();
				img.src = _src;
		...
		items:function(){
			...
			for(var i=0;i<len;i++){
				item_str += "<div class='Img-item-set' style='z-index:"+(len-i)+";background:url("+data[i].src+") center center no-repeat;'></div>";
			}
			return item_str;	
		},
		navbtn:function(){
		...
		fill_model:function(){
			var sp = "<div class="+this.config.midcon_class+"></div>";
			return sp;
		},
		fade:function(){
			var data = this.config.data,
...
Picooc.browser = (function(){
	...
	function below_ie10(){
		...
		var bro=appInfo();
		//var _version = bro.version.splite('.')[0];
		if(bro == 'IE' && parseInt(bro._version)<10){
			lessIE10();
			...
			var ind = userAgent.indexOf('rv:');
			if(ind !=-1){
				var version = userAgent.substr(ind+3,2);
