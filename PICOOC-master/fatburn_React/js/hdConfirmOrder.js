...
function getGoodsInfo(){
    ...
    $.ajax({
        ...
        success : function (data) {
            ...
            if(data.code == 200){
                ...
                //100/300
                var beginTimeArr2 = data.resp.nowDate.split(' '); //开营时间
                beginTimeArr2 = beginTimeArr2[0].split('-'); //开营时间
                $('.beginTime .year').html(beginTimeArr2[0]);//开营时间:年
                $('.beginTime .month').html(beginTimeArr2[1]);//开营时间:月
                $('.beginTime .day').html(beginTimeArr2[2]);//开营时间:日
                $('.new-price').html(currentPrice); //商品现价 ////////////////////////
                $('.jine .youHuiMoney').html('-¥0'); ///////////////////优惠金额
                $('.pay .realyPay span').html('¥'+currentPrice);//////////////////实付款
    ...
    $('#phoneNum').bind('input propertychange', function() {
        ...
        if(($("#phoneNum").val() == $("#wechatNum").val()) && ($("#phoneNum").val() != '') && ($("#wechatNum").val() != '')){
            $("#samePhone").attr("src","image/hdConfirmOrder/order-4.png");
        }else{
            $("#samePhone").attr("src","image/hdConfirmOrder/order-3.png");
        }
    ...
    $("#samePhone").unbind("click").click(function(){
        ...
        if(!PhoneBtn){
            $(this).attr("src","image/hdConfirmOrder/order-4.png");
            var phoneNum = $("#phoneNum").val();
            $("#wechatNum").val(phoneNum);
            PhoneBtn=true;
        }else{
            $(this).attr("src","image/hdConfirmOrder/order-3.png");
            $("#wechatNum").val("");
            PhoneBtn=false;
        }
                    ...
                    if(data.code == 200){
                        ...
                        if(data.resp == 1){//售罄但有未支付的订单
                            ...
                            if($(".submit span").attr("clicked") == "0"){
                                ...
                                var goodsInfo = {
                                    "goodsId":parseInt(getParamByUrl('id')),
                                    ...
                                    "couponId":null,
                                    "sourceType":"燃脂营APP",
                                    "trafficSource":trafficSource //流量来源，开屏/push/短信/banner/其他 ,  这个地方还需要做调整！！！
                                };
                                        ...
                                        if(data.code == 200){
                                            ...
                                            if(deviceType == "isApp" && (typeof mobileApp != "undefined")){
                                                var getPageInfo = function (){
                                                    var paydata = {
                                                        orderId:data.resp,
                                                        url:absoluteUrl+"hdPaySuccess.html"+window.location.search+"&orderId="+data.resp,
