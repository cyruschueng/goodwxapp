...
function getList2(type,campId){
	$(".msgType2 .list").html('');
	
	var finalUrl=ajaxLink+"/v1/api/camp/checkList"+window.location.search+"&targetRoleId="+roleId+"&type="+type+"&campId="+campId;
	$.ajax({
		type:"get",
		url:finalUrl,
		dataType:"json",
	...
	$.ajax({
		...
		success:function(data){
			if(data.code == 200){
				...
				$(".msgType2 .partRight-msg").eq(partIndex).append(str);
				bindMsg();
				bindName();
				$(".comment").css("display","none");
				$("#comment-msg").val("");
