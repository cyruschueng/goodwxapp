...
//身体基本信息子模块
var StudentOtherInfo_baseInfo =require("./StudentOtherInfo_baseInfo.jsx");
//部分页面公用参数
...
var StudentInfoTop = React.createClass({
	...
	addFriendJump:function(name,friendUserId){
			...
			return (
				<div>
	                <div className="top-info">
	                	<div className="row top-info1">
							<div className="col-xs-3 col-sm-3 top-img" style={{visibility:"visible"}}>
								<img className="head" src={data.resp.title.head} onError={imgError.bind(this,data.resp.title.sex)} />
								<img className="sex" src={headerImg} />
							</div>
							<div className="col-xs-9 col-sm-9 row right-part">
								<div className="row account">
									<span id="accountVal">{data.resp.title.name}</span>
								</div>
								<div style={{display:'none'}}>
									<span id="friendUserId">{data.resp.friendUserId}</span>
								</div>
								<div className="row addFriend" onClick={me.addFriendClick}>
									<span>+加为PICOOC好友</span>
									<span className="roleName" style={{display:'none'}}>{data.resp.roleName}</span>
									<span className="isReply" style={{display:'none'}}></span>
								</div>
								<div className="row hasAddFriend">
									<img src="http://cdn2.picooc.com/web/res/fatburn/image/studentInfo/hasAddFriend.png" />
									<span>已加为PICOOC好友</span>
								</div>
							</div>	
						</div>
    	...
    	if(typeof baseInfo.resp != "undefined"){
    		return (
	            <div className="row" id="tag-content">
	                ...
	                <StudentOtherInfo_baseInfo baseInfo={baseInfo} isPersonInfo={false} />
	            </div>
	        );
    	}
    	return (<i></i>);
    ...
    render:function (){
        ...
        return (
            <div>
                ...
                <StudentInfoCenter baseInfo={me.state.baseInfo} ref="getChartData"/>
            </div>
        );
    }
})
...
var SaveInfo = React.createClass({
	render:function (){
		return (
			<aside className="row fixbg fixbg1">
				<div className="col-xs-12 col-sm-12 fixbg-main">
					<div className="row">
						<div className="col-xs-12 col-sm-12 fixbg-main-t">您已发送请求，等待对方处理</div>
						<div className="col-xs-12 col-sm-12 fixbg-main-btn">
							<div className="row">
								<div className="col-xs-12 col-sm-12 fixbg-main-btn1">我知道了</div>
							</div>
						</div>
					</div>
				</div>
