...
function trainHistory(){
	var host=window.location.protocol+"//"+window.location.host;
	var finalUrl=host+"/v1/api/campScheme/getSportData?roleId=2&campId=2";
	...
	$.ajax({
		...
		success:function(data){
			if(data.code == 200){
				console.info(data);
				//运动时间
				data.resp.times = parseInt(data.resp.times/1000);
				var minute = data.resp.times / 60 ;
				var second = data.resp.times % 60 ;
				$(".trainTime_info .minute").text(minute);
				$(".trainTime_info .second").text(second);

				//未结营
				if(!data.resp.end){
					var trainHistoryList = data.resp.weekSportDataDTOs;
					var trainHistoryHtml = "";
					var canvasIndex = 0;
					for(var i=0;i<trainHistoryList.length;i++){
						var dayHistoryList = trainHistoryList[i].daySportDataDTOs;
						var groupHistoryHtml = "";
						for(var j=0;j<dayHistoryList.length;j++){
							console.info(dayHistoryList[j].dayNum);
							var groupHistoryList = dayHistoryList[j].sectionSportDataDTOs;
							
							for(var k=0;k<groupHistoryList.length;k++){
								var dayTrainTime = parseInt(groupHistoryList[k].times/60)+"分"+parseInt(groupHistoryList[k].times%60)+"秒";
								if(k != 0){
									dayHistoryList[j].dayNum = "";
								}
								groupHistoryHtml += '<div class="train_day_item row">'
														+'<div class="train_day_date col-xs-2 col-sm-2">'+dayHistoryList[j].dayNum+'</div>'
														+'<div class="train_day_timeInfo col-xs-4 col-sm-4">'
															+'<img src="http://cdn2.picooc.com/web/res/fatburn/image/studentInfo/trainTime.png">'
															+'<span>'+dayTrainTime+'</span>'
														+'</div>'
														+'<div class="train_day_groupInfo col-xs-3 col-sm-3">'
															+'<img src="http://cdn2.picooc.com/web/res/fatburn/image/studentInfo/fire.png">'
															+'<span>'+groupHistoryList[k].groups+'组</span>'
														+'</div>'
														+'<div class="train_day_rateInfo col-xs-3 col-sm-3">'
															+'<canvas id="canvas'+canvasIndex+'" class="canvas_bg" percent="'+groupHistoryList[k].percent+'"></canvas>'
															+'<span class="parcentText"><span>'+groupHistoryList[k].percent+'</span>%</span>'
						...
						trainHistoryHtml += '<div class="train_week_list">'
												+'<div class="train_week_title">'+trainHistoryList[i].weekNum+' '+trainHistoryList[i].startDate+'-'+trainHistoryList[i].endDate+'</div>'
												+'<div class="train_day_list row">'+groupHistoryHtml+'</div>'
											+'</div>';
					}
				}
				//已结营
				else{
					var trainHistoryList = data.resp.endSportDataDTOs;
					var trainHistoryHtml = "";
					var canvasIndex = 0;
					trainHistoryHtml += '<div class="train_week_title">已结营</div>';
					trainHistoryHtml += '<div class="train_day_list row">';
					for(var i=0;i<trainHistoryList.length;i++){
						var groupHistoryList = trainHistoryList[i].sectionSportDataDTOs;
						for(var j=0;j<groupHistoryList.length;j++){
							var dayTrainTime = parseInt(groupHistoryList[j].times/60)+"分"+parseInt(groupHistoryList[j].times%60)+"秒";
							if(j != 0){
								trainHistoryList[i].dateNum = "";
							}
							trainHistoryHtml += '<div class="train_day_item row">'
													+'<div class="train_day_date col-xs-2 col-sm-2">'+trainHistoryList[i].dateNum+'</div>'
													+'<div class="train_day_timeInfo col-xs-4 col-sm-4">'
														+'<img src="http://cdn2.picooc.com/web/res/fatburn/image/studentInfo/trainTime.png">'
														+'<span>'+dayTrainTime+'</span>'
													+'</div>'
													+'<div class="train_day_groupInfo col-xs-3 col-sm-3">'
														+'<img src="http://cdn2.picooc.com/web/res/fatburn/image/studentInfo/fire.png">'
														+'<span>'+groupHistoryList[j].groups+'组</span>'
													+'</div>'
													+'<div class="train_day_rateInfo col-xs-3 col-sm-3">'
														+'<canvas id="canvas'+canvasIndex+'" class="canvas_bg" percent="'+groupHistoryList[j].percent+'"></canvas>'
														+'<span class="parcentText"><span>'+groupHistoryList[j].percent+'</span>%</span>'
														+'<img src="http://cdn2.picooc.com/web/res/fatburn/image/studentInfo/processText.png" />'
													+'</div>'
												+'</div>';
							canvasIndex++ ;
					...
					trainHistoryHtml += "</div>";
				}
				$(".trainList").html(trainHistoryHtml);
				$(".canvas_bg").each(function(index){
					var canvasIndex = $(this).attr("id");
					var percent = $(this).attr("percent");
					drawRate(canvasObj.startX,canvasObj.startY,canvasObj.radius,canvasObj.innerRadius,canvasObj.startAngle,canvasObj.endAngle,canvasIndex,percent);
