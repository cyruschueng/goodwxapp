...
function getImg(url){
	...
	for(var i=0;i<url.length;i++){
		...
		//getImgStr+='<div class="col-xs-4 col-sm-4 getImg-img"><div class="getImg-bg" style="background-image:url(\''+url[i]+'\')"></div><img class="getImg-delete" src="image/setcard/setcard-10.png" /></div>';
		//$(".getImg-add").before('<div class="col-xs-4 col-sm-4 getImg-img"><div class="getImg-bg" style="background-image:url(\''+url[i]+'@300h_300w_1e_1c\')"></div><img class="getImg-delete" src="image/setcard/setcard-10.png" /></div>');
		getImgStr+='<div class="col-xs-4 col-sm-4 getImg-img"><img class="getImg-bg" src="'+url[i]+'@500h_500w_1e_1c" onerror="imgError(this);" /><img class="getImg-delete" src="image/setcard/setcard-10.png" /></div>';
		getImgStr2+='<div class="col-xs-12 col-sm-12 swiper-slide bigImg-li" style=\"background-image:url('+url[i]+')\"></div>';
	}
	
	$(".getImg-add").before(getImgStr);
	
	//$("#msgInfo").val(url[0]+'|'+url[1]+'|'+url[2]);
	/*$(".getImg").prepend(str);
    ...
    if(getParamByUrl("os")=="android"){
        $(".bigImg-li").css("height",$(window).height()+70);
    }
    else{
        $(".bigImg-li").css("height",$(window).height()+64);
    }
...
function imgError(ev){
	var index=0;
	console.log(ev.src.split("?")[1]);
	if(ev.src.split("?")[1]!="" && ev.src.split("?")[1]!=undefined){
		index=ev.src.split("?")[1];
	}
	console.log(parseInt(index));
	if(parseInt(index)<10){
		//var time=new Date().getTime();
		
		ev.src=ev.src.split("?")[0]+'?'+parseInt(index)+1;
	}
