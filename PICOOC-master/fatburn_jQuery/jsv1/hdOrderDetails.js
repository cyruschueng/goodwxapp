        ...
        success:function(data){
          if(data.code == 200){
            if(data.resp!=undefined && data.resp!=null && data.resp!=""){
                ...
                var buyTime = (data.resp.createTime).substring(0,10);
                var arr=buyTime.split('-');
                var buyTime2 = arr[0]+'年'+arr[1]+'月'+arr[2]+'日';
                $("#beginTime").html(buyTime2);
                    ...
                    if(deviceType == "isApp" && (typeof mobileApp != "undefined")){
                      var getPageInfo = function (){
                        var paydata = {
                          ...
                          url:absoluteUrl+"hdPaySuccess.html"+window.location.search,
