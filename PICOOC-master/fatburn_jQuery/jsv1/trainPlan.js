			...
			if(data.code == 200){
				if(data.resp.length > 0){
					...
					for(var i=0;i<data.resp.length;i++){
						...
						trainHtml+='<div onclick="trainPlan('+data.resp[i].weekIndex+',0)" class="sportPlan-item">'+data.resp[i].weekNum+'&nbsp;&nbsp;'+data.resp[i].title+'</div>';
					}
					$(".part1").empty();
...
function trainPlan(weekIndex){
	setMaiDian(SWoDeFangAn.SCategory_SWoDeFangAn,SWoDeFangAn.SWoDeFangAn_FangAnXiangQing);
	getWeekTrain(weekIndex);
}

/*客户端联调*/

//获得一周的运动计划
function getWeekTrain(weekIndex){
...
            roleId: roleId,
	...
	if(getParamByUrl("os")=="android"){
		...
		mobileApp.getWeekTrain.postMessage(getPageInfo());
	}
    document.documentElement.style.webkitTouchCallout='none';
}
