...
$(function(){
    ...
    //alert(getCookie("clickSave"));
    if(getCookie("clickSave") == 1){
        $('.page1,.page2').hide();
        $('.page3').show();
        $(".navBar").css('left', "18.984375rem");
    }
    delCookie("clickSave");
	/*页面显示tag--开始*/
	$(".info-tag .info-tag-item").each(function(index){
		$(this).unbind("click").click(function(){
			...
			if(nextIndex == 0){
				...
				$(".noJump input").unbind("touchstart").bind("touchstart",function(){
					this.blur();
				});
				$(".noJump input").focus(function(){
			        this.blur();
			    });
			}
