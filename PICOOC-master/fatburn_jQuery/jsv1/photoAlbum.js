			...
			success:function(data){
				if(data.code == 200){
					...
					if(data.resp.data.length>0){
						...
						// }
						$(".msg").css("margin-bottom",$(".upload-container").height()+10);
						//显示预览图
						...
						//编辑图片
						$(".manage .edit").unbind("click").click(function(event){
							var campPictureId = $(this).attr("campPictureId");
							// var deviceType=isMobile();//判断是不是app的方法
							// if(deviceType == "isApp" && (typeof mobileApp != "undefined") && (getParamByUrl("testtype")!="tanchao")){
							...
							// }else{
								...
								if(getParamByUrl("imgId")!="false"){
									...
									window.location.href=absoluteUrl+"editPhoto.html"+searchLink+"&imgId="+campPictureId;
								}
								else{
									window.location.href=absoluteUrl+"editPhoto.html"+window.location.search+"&imgId="+campPictureId;
								}
...
if(deviceType == "isApp" && (typeof mobileApp != "undefined")){
 ...
 function rightBtnShow(){
	...
	if(deviceType == "isApp" && (typeof mobileApp != "undefined")){
		if(getParamByUrl("os")=="android"){
			...
			mobileApp.showRightBtn.postMessage(data);
		}
		//mobileApp.showRightBtn(data);
	}
 ...
 function getControl1(){
	...
	if(deviceType == "isApp" && (typeof mobileApp != "undefined")){
	 ...
	 if(deviceType == "isApp" && (typeof mobileApp != "undefined")){
		 if(getParamByUrl("os")=="android"){
			 mobileApp.controlRightInfo(getPageInfo1());
		 }
		 else{
			 window.webkit.messageHandlers.controlRightInfo.postMessage(getPageInfo1());
		 }
	 }
 }
...
function controlOver(){
 ...
 function appNoShare(){
	...
	var deviceType=isMobile();
		if(deviceType == "isApp" && (typeof mobileApp != "undefined")){
			if(getParamByUrl("os")=="android"){
				mobileApp.controlTitle(getPageInfo1());
			}
			else{
				mobileApp.controlTitle.postMessage(getPageInfo1());
			}
