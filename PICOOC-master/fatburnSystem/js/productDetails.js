...
$(function(){
    ...
    if(getParamByUrl('typeSize') == 2){ //100抵300优惠券售卖
            var finalUrl=ajaxLink +"/v1/api/campAdmin/getGoodsInfo"+window.location.search;//获取商品信息
            $.ajax({
                type: "get",
                url: finalUrl,
                dataType:"json",
                success: function(data){
                    ...
                    //                 //shareUrl:sharelink+"index.html?type=share",
    ...
    //delCookie('youHuiState');//删除选择优惠券时定义的cookie
    //$('.info .infoRight .times').hide().eq(3).show();//库存：5个状态
    //$('.desc .begin').hide().eq(0).show(); //开营时间：2个状态
    //$('.sale .state').hide().eq(0).show(); ////开售提醒：5个状态
    //$('.sale .service img').hide().eq(0).show(); //客服：2个状态
    //$('.alertBox .alerts').hide().eq(2).show(); //灰色弹出框：3个状态
    //$('.alertBox .alerts2').hide().eq(0).show(); //立即预约、版本过低弹出框：2个状态
    ...
    $('#btn1').on('click', function(){
        ...
        $('.info .infoRight .times').hide().eq(num1).show();
    });
    ...
    $('#btn3').on('click', function(){
        ...
        $('.sale .service img').hide().eq(num3).show();
    });
    ...
    $('#btn5').on('click', function(){
        ...
        $('.alertBox .alerts').hide().eq(num5).show();
    });
    $('#btn52').on('click', function(){
        $('.alertBox .alerts').hide();
    });
    $('#btn6').on('click', function(){

        $('.alertBox .alerts2').toggle();
    });*/
...
// $(".service, .hdService").unbind("click").click(function () {
//     if(getParamByUrl('type') != 2){//燃脂营售卖客服系统埋点
            ...
            /*var deviceType=isMobile();
             if(deviceType == "isApp" && (typeof mobileApp != "undefined")){*/
                ...
                if(data.result.code == 200){
                    ...
                    $('.info, .desc, .service, .hdService').show();
                    ...
                    //alert(1);
                    //console.log($bannerPoint.css('height'));
                    //console.log($bannerPoint.css('width'));
                    //console.log($bannerPoint.css('borderRadius'));
                    ...
                    function fatBurnVersion(){
                        if(getParamByUrl("os")=="iOS"){// 判断如果是ios
                            ...
                            $('.alerts2 .isUpdate .updata').unbind('click').click(function(){
                                // setMaiDian(SShangPinXiangQing.SCategory_SShangPinXiangQing,SShangPinXiangQing.SShangPinXiangQing_ShengJiBanBen);//版本升级埋点
                                window.location.href = 'http://a.app.qq.com/o/simple.jsp?pkgname=com.picooc&g_f=991653'; //跳转到APP版本升级页面
                            });
            ...
            function appShare3(){
                ...
                var getPageInfo = function (){
                    ...
                    /!*var data2 = {
                        ...
                        backgroundColor:'#2c2f31',
                        isShare:true,
                        shareTitle:"获奖公布每个小目标都是你享瘦2017的开始~",
                        shareUrl:"http://detection.picooc.com/details388",
                        shareIcon:"http://detection.picooc.com/data/upload/portal/share/2017-01-05/586e1c1ad37d4.jpg",
                        shareDesc:"无论中奖与否新的一年可得要好好对待哦~",
                        shareTag:"share_details_388"
                    };*!/
                    //$("#test11").html(data2);
                    //alert(data2.shareTitle+"|"+data2.shareUrl+"|"+data2.shareIcon+"|"+data2.shareDesc+"|"+data2.shareTag);
...
//获取时间差 (100抵300使用)
//timeDiffer2("2016-11-14 10:00:00", "2016-11-14 10:00:10");
function timeDiffer2(nowTime, endTime) {
    if(getParamByUrl("os")=="iOS"){// 判断如果是ios，显示2016/11/14的格式；
        endTime = endTime.replace(/-/g,"/");
        nowTime = nowTime.replace(/-/g,"/");
    }else{}
...
// var finalUrl2=ajaxLink +"/v1/api/campGoods/getGoodsInfo"+window.location.search;//获取商品信息
// $.ajax({
//     type: "get",
//     url: finalUrl2,
//     dataType:"json",
