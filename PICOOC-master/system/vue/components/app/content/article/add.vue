<template>
	<div class="articleAdd">
		<div class="title">管理后台 > APP > 内容管理 > 文章管理 > 新增</div>
...
export default {
	...
	methods:{
		...
		addArticle:()=>{
			...
			if($(".articleAbstract").val().length>100){
				alert("文章摘要最长不超过100字");
				return true;
			}
			if($(".articleShareTitle").val().length>100){
				alert("文章分享标题最长不超过100字");
				return true;
			}
			if($(".articleShareInfo").val().length>100){
				alert("文章分享描述最长不超过100字");
				return true;
			}
			let ajaxData={};
			...
			$.ajax({
				...
				success:function(data){
					if(data.code == 200){
						alert(data.msg);
						router.push({ path: 'appContentArticleIndex' });
					}
					else{
						alert(data.msg);
					}
