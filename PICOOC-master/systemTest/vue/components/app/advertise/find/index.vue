<template>
	<div class="findIndex">
		<div class="title">管理后台 > APP > 广告管理 > 电商的发现轮播图管理</div>
		<search  @search="searchFun"/>
    <aside class="tabTop">
      <router-link class="add" :to="{name:'appAdvertiseFindAdd'}">新增</router-link>
      <div class="btn btn1 btnActive" @click="getFindList(1,pageSize,'','')">全部</div>
      <div class="btn btn2" @click="getFindList(1,pageSize,'',0)">编辑中</div>
      <div class="btn btn3" @click="getFindList(1,pageSize,'',1)">待审核</div>
      <div class="btn btn4" @click="getFindList(1,pageSize,'',2)">待发布</div>
      <div class="btn btn5" @click="getFindList(1,pageSize,'',3)">已发布</div>
        ...
        <div class="tabOpenType" v-if="((item.link=='') || (item.link==null))"></div>
          ...
          <span v-if="item.status==3">已发布</span>
            ...
            <div class="tabOperation">
                <div class="change"><router-link class="edit" :to="{name:'appAdvertiseFindAdd',query: {id: item.id}}">编辑</router-link></div>
...
<script>
    ...
    export default {
        ...
        created: function () {
            getWindowSearch();
            this.getFindList(this.currentPage, this.pageSize, this.title, this.status);
        },
        methods: {
            ...
            searchFun(keyWords){
                this.title = keyWords;
                this.getFindList(1, this.pageSize, keyWords, '');
            },
            //电商的发现轮播图管理列表
            getFindList: function (currentPage, pageSize, title, status) {

                $(function () {
                    $(".findIndex .btn").unbind("click").click(function () {
                        var index = $(".findIndex .btn").index(this);
                        $(".findIndex .btn").removeClass("btnActive");
                        $(".findIndex .btn").eq(index).addClass("btnActive");
                    });
                });

                let _this = this;
                let finalUrl = ajaxLink + 'picooc-background/carouselFigureAd/list?' + token + windowSearch + '&currentPage=' + currentPage + '&pageSize=' + pageSize + '&title=' + title + '&status=' + status;
            ...
            submitItem: function (id, index) {
                let _this = this;
                let finalUrl = ajaxLink + 'picooc-background/carouselFigureAd/submitAudit?' + token + windowSearch + '&id=' + id;
                $.ajax({
                    type: 'get',
                    url: finalUrl,
                    success: function (data) {
            ...
            passItem: function (id, index) {
                let _this = this;
                let finalUrl = ajaxLink + 'picooc-background/carouselFigureAd/auditPass?' + token + windowSearch + '&id=' + id;
                $.ajax({
                    type: 'get',
                    url: finalUrl,
                    success: function (data) {
            ...
            passNotItem: function (id, index) {
                let _this = this;
                let finalUrl = ajaxLink + 'picooc-background/carouselFigureAd/auditReject?' + token + windowSearch + '&id=' + id;
                $.ajax({
                    type: 'get',
                    url: finalUrl,
                    success: function (data) {
            ...
            publishNotItem: function (id, index) {
                let _this = this;
                let finalUrl = ajaxLink + 'picooc-background/carouselFigureAd/publish?' + token + windowSearch + '&id=' + id;
                $.ajax({
                    type: 'get',
                    url: finalUrl,
                    success: function (data) {
            ...
            recallItem: function (id, index) {
                let _this = this;
                let finalUrl = ajaxLink + 'picooc-background/carouselFigureAd/unPublish?' + token + windowSearch + '&id=' + id;
                $.ajax({
                    type: 'get',
                    url: finalUrl,
                    success: function (data) {
            ...
            deleteItem: function (confirmData) {
                ...
                let _this = this;
                let finalUrl = ajaxLink + 'picooc-background/carouselFigureAd/delete?' + token + windowSearch + '&id=' + id;
                $.ajax({
                    type: 'get',
                    url: finalUrl,
                    success: function (data) {
