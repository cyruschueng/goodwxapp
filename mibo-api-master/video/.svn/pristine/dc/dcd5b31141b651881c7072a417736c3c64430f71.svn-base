    ...
    public function getAppConfig($os, $channel='', $version_code = 0) {
        ...
        if(empty($default_os_config)) {
            $default_os_config = $this->getORM()->where('os = '.$os.' and channel = "" and version_code = 0')
                ->fetchPairs("event");
            //DI()->redis->set_time('app_default_'.$os.'_config', $default_os_config, 3600);
        }
        //print_r($default_os_config);exit;
        ...
        if(empty($default_channel_config)) {
            ...
            //DI()->redis->set_time('app_default_'.$channel.'_config', $default_channel_config, 3600);
        }
        ...
        if(empty($version_config)) {
            ...
            //DI()->redis->set_time('app_'.$os.'_'.$channel.'_'.$version_code.'_config', $version_config, 3600);
        }

        //$default_channel_config = DI()->redis->get_time('app_default_channel_config');
        //if(empty($default_channel_config)) {
        //    $default_channel_config = $this->getORM()->where('os = '.$os)->where('channel = ""')->where('version_code = 0')
        //        ->fetchPairs("event");
        //    DI()->redis->set_time('app_default_config', $default_config, 3600);
        //}
        //
        //$channel_config_arr = DI()->redis->get_time('app_'.$os.'_'.$channel.'_config');
        //if(empty($channel_config_arr) || !isset($channel_config_arr[$version_code])) {
        //    $channel_config = $this->getORM()->where('os = ' . $os)->where('channel = "' . $channel .'"')
        //        ->where('version_code = 0 or version_code = ' . $version_code)->fetchPairs("event");
        //    $channel_config_arr[$version_code] = $channel_config;
        //    DI()->redis->set_time('app_'.$os.'_'.$channel.'_config', $channel_config_arr, 3600);
        //}
        //print_r($channel_config_arr);exit;
