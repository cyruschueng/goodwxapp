...
class Redis_Lite extends PhalApi_Cache_Redis{
    ...
    public function set($key, $value, $expire = 600, $dbname = 0){
        if(is_array($value)) {
    ...
    public function get_int($key, $dbname = 0) {
        $this->switchDB($dbname);
     ...
     */
    public function lRem($key, $value, $count = 0, $dbname = 0){
        $this->switchDb($dbname);
        $value = $this->formatValue($value);

        $rs = $this->redis->lRem($this->formatKey($key), $value, $count);
        return $rs;
    }
     ...
     * 返回满足给定pattern的所有key
     */
    public function keys($pattern, $dbname = 0){
        $this->switchDB($dbname);
        return $this->redis->keys($this->formatKey($pattern));
    }
    ...
    //添加hash值
    public function hSet($key, $field, $value, $expire = 43200) {
        $key = $this->formatKey($key);
    ...
    //向名称为key的set中添加元素value,7天过期
    public function sAdd($key, $value, $expire = 604800) {
        $key = $this->formatKey($key);
    ...
    //向名称为key的zset中添加元素value   return int 1
    public function zAdd($key, $score, $value) {
        $key = $this->formatKey($key);
        return $this->redis->zAdd($key, $score, $value);
    }

    //根据索引取得有序集合的集，注意返回的数组分数是作为值的
    public function zRange($key, $start, $end, $bool = true) {
        $key = $this->formatKey($key);
        return $this->redis->zRange($key, $start, $end, $bool);
    }

    //根据分数取得有序集合的集，注意返回的数组分数是作为值的
