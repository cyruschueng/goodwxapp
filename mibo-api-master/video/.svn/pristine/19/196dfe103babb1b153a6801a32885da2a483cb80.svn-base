...
//        }
    ...
    public function registerByPhone() {
        ...
        $user_data['avatar'] = 'http://image.cdn.mibolive.com/img/default/ic_head.png';
    ...
    public function setPassword() {
        ...
        $url = DI()->config->get('sys.api_url_host').'/mibo30/index.php?phone=' . $phone .
    ...
    public function updateWealth($req) {
        $model_user = new Model_User();
        $user_info = $model_user->getUsersInfoById($req['user_id']);

        switch($req['type']) {
            case 1:
                $before_num = $user_info['coin_num'];
                break;
            case 2:
                $before_num = $user_info['diamond_num'];
                break;
            default:
                $before_num = $user_info['coin_num'];
        }

        $after_num = $before_num + $req['num'];
        ...
        DI()->notorm->user_wealth_log->insert($log_data);
        $insert_id = DI()->notorm->user_wealth_log->insert_id();

        if(!$insert_id) throw new PhalApi_Exception('操作失败', 462);

        if($req['type'] == 1) {
            $rs = $model_user->updateUserDiamondCoin($req['user_id'], 0, $req['num']);
        }

        if($req['type'] == 2) {
            $rs = $model_user->updateUserDiamondCoin($req['user_id'], $req['num'], 0);
        }

        return (isset($rs) && $rs) ? true : false;
     ...
     * @return bool
     * @ignore
     * @request http://t.com/mibotest.yahalei.com/public/mibo32/index.php?service=user.updateVip&user_id=11773&vip_level=1
     */
    public function updateVip($req) {
        $data['vip_level'] = $req['vip_level'];
        $rs = DI()->notorm->user->where('id', $req['user_id'])->update($data);
