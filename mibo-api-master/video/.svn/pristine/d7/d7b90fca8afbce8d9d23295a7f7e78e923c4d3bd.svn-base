...
class Api_Default extends PhalApi_Api {
    ...
    public function test() {
        ...
        return $domain->liveEnd();

        $data['dealer_id'] = 2;
        $data['dealer_card'] = json_encode($game->getCard());
        $data['pool1_card'] = json_encode($game->getCard());
        $data['pool2_card'] = json_encode($game->getCard());
        $data['pool3_card'] = json_encode($game->getCard());

//        $data['dealer_card'] = '[["1","10","\u9ed1\u6843-10"],["2","Q","\u7ea2\u6843-Q"],["4","2","\u65b9\u5757-2"],["3","J","\u6885\u82b1-J"],["2","7","\u7ea2\u6843-7"]]';
//        $data['pool1_card'] = '[["1","4","\u9ed1\u6843-4"],["3","K","\u6885\u82b1-K"],["1","6","\u9ed1\u6843-6"],["4","A","\u65b9\u5757-A"],["1","3","\u9ed1\u6843-3"]]';
//        $data['pool2_card'] = '[["2","A","\u7ea2\u6843-A"],["1","K","\u9ed1\u6843-K"],["3","9","\u6885\u82b1-9"],["3","5","\u6885\u82b1-5"],["4","3","\u65b9\u5757-3"]]';
//        $data['pool3_card'] = '[["4","Q","\u65b9\u5757-Q"],["1","9","\u9ed1\u6843-9"],["2","6","\u7ea2\u6843-6"],["1","Q","\u9ed1\u6843-Q"],["4","9","\u65b9\u5757-9"]]';

        var_dump($data);
        $rs = $game->compareCards(json_decode($data['pool1_card']),json_decode($data['pool2_card']));
        var_dump($rs);
...
//        var_dump($rs);
    ...
    public function testMsg() {
        ...
        $domain_im = new Domain_IM();
        $rs = $domain_im->sendLiveMsg(1, '13024207372289', '恭喜！', IM_GAME_BIG_WINNER);exit;
        //$room_info = $easemob->getChatRoomDetail('267675055385740868');
        $room_info = $easemob->getChatRoomDetail('5960187838465');
        print_r($room_info);exit;

        //$live_lists = DI()->notorm->live_list->select('id, chatroom_id')->where('status = 2 and type = 1')->fetchAll();
        //foreach($live_lists as &$live) {
        //    $room_info = $easemob->getChatRoomDetail($live['chatroom_id']);
        //    $live['affiliations_count'] = $room_info['data'][0]['affiliations_count'];
        //}

        print_r($live_lists);exit;
    ...
    public function sendMsg() {
        ...
        $domain = new Domain_Crond();
        $domain->sendBigGift();exit;
...
//        $rs = $im_domain->sendUserMsg(2, 11660, $msg);
    ...
    public function testSql() {
        ...
        $deal_rows = DI()->notorm->user_deal_log
            ...
            ->limit(1)
            ->fetchRow();
        echo count($deal_rows);exit;
        var_dump($deal_rows);exit;
    }
