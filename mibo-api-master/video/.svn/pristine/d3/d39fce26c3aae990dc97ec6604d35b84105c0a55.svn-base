...
class Domain_Personal extends Domain_Common{
    ...
    public function identifyName() {
        ...
        if($user_info['anchor_type'] > 0) throw new PhalApi_Exception('此用户已经是主播，有问题联系客服！', 479);

        $had_identified = DI()->notorm->user_id_card->where('user_id', $user_id)->fetchOne();
        if(!empty($had_identified)) throw new PhalApi_Exception('此用户已经认证过, 有问题联系客服！', 480);
    ...
    public function getFansList() {
        ...
        foreach ($user_list as &$user) {
            ...
            //粉丝用户列表加上关注时间
            foreach($fans_list as $per_fans) {
                if($per_fans['user_id'] == $user['id']) {
                    $user['follow_time'] = $per_fans['create_time'];
                }
            }

        }

        $sort = array_column($user_list, 'follow_time');
        array_multisort($sort, SORT_DESC, $user_list);

        array_values($user_list);
    ...
    public function getFollowList() {
        ...
        foreach($follow_list as $perfollow) {
            ...
            if(!empty($perfollow['my_follow_id'])) {    //是否是我的粉丝
        ...
        if(!empty($user_list)) {
            foreach ($user_list as &$user) {
                ...
                //关注用户列表信息加上关注时间
                foreach($follow_list as $per_follow) {
                    if($per_follow['to_user_id'] == $user['id']) {
                        $user['follow_time'] = $per_follow['create_time'];
                        break;
                    }
                }
    ...
    public function guardList() {
        ...
        if(empty($guard_list)) {
            ...
            if($self_info['is_anchor'] == 1) {
                ...
                DI()->redis->set_time('guard_list_' . $this->req['user_id'], $redis_guard_list, 5);
            }
