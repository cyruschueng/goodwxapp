...
//------------------------------------------------------用户体系	
    ...
    */
    ...
    /*
        获取app中所有的聊天室
    */
    function getChatRooms($page_no = 1)
    {
        $url = $this->url . 'chatrooms?pagenum=' . $page_no . '&pagesize=100';
        $header = array($this->getToken());
        $result = $this->postCurl($url, '', $header, "GET");
        return $result;
    }
    ...
    */
    ...
    /*
        聊天室管理员列表
     */
    function getChatRoomAdmin($chatroom_id) {
    ...
    /*
        聊天室单个管理员添加
    */
    function addChatRoomAdmin($chatroom_id, $username)
    {
        $url = $this->url . 'chatrooms/' . $chatroom_id . '/admin';
        $header = array($this->getToken());

        $body = ['newadmin' => $username];
     ...
     * 禁言列表
     */
    function getRoomMuteList($chatroom_id) {
    ...
    */
    function getChatRecord($time)
    {
        if (!empty($time)) {
            ...
            $url=$this->url.'chatmessages/'.date('YmdH', strtotime('-3 hours'));
        }
        $header = array($this->getToken());
        $result = $this->postCurl($url, '', $header, "GET");
        return $result;
