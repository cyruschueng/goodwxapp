...
class Domain_Crond {
    ...
    public function robot() {
        ...
        foreach ($live_list as $live) {
            ...
            $user_entry_url = DI()->config->get('sys.api_url_host')."/mibo30/index.php?service=Live.userEntry&channel=bt&user_id=" . $robot_user_info['id'] . "&live_id=" . $live['id'];
        ...
        foreach ($live_lists as $live_info) {
            ...
            if (isset($exit_num) && $exit_num > 0) {
                ...
                if (!empty($robot_ids_arr)) {
                    ...
                    $user_exit_url = DI()->config->get('sys.api_url_host')."/mibo30/index.php?service=Live.userExit&channel=bt&user_id=" . $robot_id . "&live_id=" . $live_info['id'];
        ...
        if (!empty($live_info_list)) {
            ...
            foreach($live_info_list as $live_info) {
                ...
                if(isset($live_game['gid'])) {
                    ...
                    $rs = @file_get_contents(DI()->config->get('sys.api_url_host').'/mibo30/index.php?service=Game.ReqCard&mibo&channel=bt&gid='.
