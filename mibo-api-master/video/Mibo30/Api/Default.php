...
class Api_Default extends PhalApi_Api {
    ...
    public function test() {
        ...
        //$sql = 'select id, receive_num from mb_user where anchor_type = 1';
        $list = DI()->notorm->user->select('id, receive_num')->where('anchor_type', 1)->fetchAll();

        $sql = 'insert into mb_withdraw (user_id, withdraw_diamond, withdraw_money, status, pre_diamond, after_diamond)
                  VALUES ';

        foreach($list as $user) {
            $sql .= '(' . $user['id'];
            $sql .= ',' .  $user['receive_num'];
            $sql .= ',0,1,' . $user['receive_num'];
            $sql .= ',0),';
        }
    ...
    public function test1() {
        ...
        $ss = DI()->redis->get_rPop('notify_fans_when_live_id_4575');
        print_r($ss);exit;
...
//        $this->notifyFansWhenLive(3243,503);
    ...
    public function testRedis() {
        $redis_key = 'replay_playing_num_live_id_6';
        $a = DI()->redis->get_int($redis_key);
        var_dump($a);exit;
...
//        $rs = $im_domain->sendUserMsg(2, 11660, $msg);
    ...
    public function testSql() {
        ...
        $mb_id = DI()->request->get('mb_id');
        if(!$mb_id) {
            throw new PhalApi_Exception('米播id不对！', 488);
        }
        $user = DI()->notorm->user->select('id, diamond_num')->where('mb_id', $mb_id)->fetchOne();
        $user_id = $user['id'];

        if($user['diamond_num'] >= 1000000) {
            throw new PhalApi_Exception('钻石充足', 489);
        }

        $num = DI()->request->get('num');
        $num = ($num == 10000) ? 10000 : 20000;

        $model_user = new Model_User();
        $rs = $model_user->updateUserDiamondCoin($user_id, $num, 0);
