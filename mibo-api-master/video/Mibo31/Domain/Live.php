...
class Domain_Live extends Domain_Common {
    ...
    public function anchorEntry() {
        ...
        //测试人员进行测试直播操作
        $domain_test = new Domain_InternalTest();
        $is_staff = $domain_test->isInternalStaff($anchor_info['mb_id']);
        if($is_staff) {
            return $domain_test->anchorEntry($live_info, $anchor_info);
        }
        ...
        $update_data = array(
            ...
            'replay_url'  => DI()->config->get('app.aliyun.record_url') . $stream_name . ".m3u8?auth_key=" . $m3u8_authkey,
    ...
    //通知主播的粉丝，主播开直播了
    public function notifyFansWhenLive2($anchor_info, $live_info) {


        $data['anchor_info']['id'] = $anchor_info['id'];
        $data['anchor_info']['nick_name'] = $anchor_info['nick_name'];

        $data['live_info']['id'] = $live_info['id'];
        $data['live_info']['cover_url'] = $live_info['cover_url'];
        $data['live_info']['push_url'] = $live_info['push_url'];
        $data['live_info']['look_url'] = $live_info['look_url'];
        $data['live_info']['m3u8_url'] = $live_info['m3u8_url'];
        $data['live_info']['chatroom_id'] = $live_info['chatroom_id'];
        $data['live_info']['location'] = $live_info['location'];

        DI()->redis->set_lPush('notify_fans_when_live_id_'.$live_info['id'], $data);
    ...
    public function notifyFansWhenLive($anchor_info, $live_info) {
        ...
        DI()->redis->set_lPush('notify_fans_when_live', $data);
    }
    ...
    public function instantLive() {
        ...
        $preview_num = DI()->notorm->live_list->where('anchor_id = ?', $this->req['user_id'])
            ->where('status = 1 or status = 2 or status = 4')->count('id');

        if ($preview_num >= 1) {
            throw new PhalApi_Exception('有未完成的直播，请到个人主页开播!', 461);
        }

        //测试人员进行测试直播操作
        $domain_test = new Domain_InternalTest();
        $is_staff = $domain_test->isInternalStaff($user_info['mb_id']);
        if($is_staff) {
            return $domain_test->instantLive($user_info);
        }
    ...
    public function anchorExit() {
        ...
        if($live_info['affiliations_count'] <= 0) {
            $see_num = DI()->redis->get_time('affiliations_count_live_id_'.$this->req['live_id']);
            $live_info['affiliations_count'] = $see_num ? $see_num : 0;
        }
    ...
    public function userEntry() {
        ...
        //二、庄家相关信息
        $this->dealGame($live_info, $user_info);

        //三、如果是无主播场
        if($live_info['type'] == 1) {
            $domain_no_anchor_live = new Domain_NoAnchorLive();
            return $domain_no_anchor_live->userEntry($live_info, $user_info);
        }
        ...
        if (!empty($get_live_info_userentry)) {
            ...
            $domain_game = new Domain_Game();
            $dealer_info = $domain_game->getDealerByLiveId($live_info['id']);
            $dealer_info = !empty($dealer_info) ? $dealer_info : $final_data['anchor_info'];
            $final_data['dealer_info'] = $dealer_info;
            ...
            //d,是否管理员
            $domain_admin = new Domain_RoomAdmin();
            $is_admin = $domain_admin->adminEntryLive($user_info, $final_data['anchor_info'], $live_info);

            $final_data['is_admin'] = $is_admin;
            ...
            $model_app_config = new Model_AppConfig();
            $msgBody['content'] = $model_app_config->getEntryNoticeContent();
            ...
            $domain_game = new Domain_Game();
            $dealer_info = $domain_game->getDealerByLiveId($live_info['id']);
            $dealer_info = !empty($dealer_info) ? $dealer_info : $anchor_info;
            ...
            //十、是否管理员
            $domain_admin = new Domain_RoomAdmin();
            $is_admin = $domain_admin->adminEntryLive($user_info, $anchor_info, $live_info);
    ...
    public function getLiveSendNum($live_id, $user_id) {
        ...
        if(empty($send_num_list)) {
            ...
            DI()->redis->set_time('send_num_list_live_id_'.$live_id, $send_num_list, 300);
        };
    ...
    public function getLastCardInfo($live_id) {
        ...
        $last_loop_card_info = $game_model->getLastLoopCards($live_id);
        if(empty($last_loop_card_info)) return NULL;

        $last_loop_card_info['gid'] = (string)$last_loop_card_info['gid'];
    ...
    public function dealGame($live_info, $user_info) {

        $had_applied_key = 'apply_dealer_user_id_'.$user_info['id'];
        DI()->redis->del($had_applied_key);

        $dealer_list_key  = 'live_id_' . $live_info['id'] . '_dealer_id_list';
        DI()->redis->lRem($dealer_list_key, $user_info['id']);
    ...
    public function sendDiamondGift($gift_info) {
        ...
        $insert_id = $deal_log_model->insertDealLog($deal_log_data);
        if ($insert_id > 0) {
            if($gift_info['price'] > 100 || $this->req['gift_num'] > 98) {
    ...
    public function liveEnd() {
        ...
        if (!empty($info)) {
            ...
            //if($live_info['status'] != 3) throw new PhalApi_Exception('直播未结束', 481);
            ...
            if($see_num <= 0) {
                ...
                $see_num = isset($room_info['data'][0]['affiliations_count']) ?
                    $room_info['data'][0]['affiliations_count'] : 0;
            ...
            //缓存给直播间的其他用户查看结束信息
            DI()->redis->set_time('live_end_info' . $this->req['live_id'], json_encode($info), 300);
    ...
    public function setLivePreview() {
        ...
        if ($preview_num >= 1) {
            throw new PhalApi_Exception('有未完成直播，请到个人主页点击播放', 356);
        }
