...
class Model_AppConfig extends PhalApi_Model_NotORM {
    ...
    public function getEntryNoticeContent() {

        $info = DI()->notorm->app_config->select('event_data')->where('event', 'user_entry_notice_content')->fetchOne();

        $content = isset($info['event_data']) && !empty($info)
            ? $info['event_data'] : '【公告】倡导绿色文明直播，米播严禁利用游戏进行赌博，违者封号，谢谢配合！';
