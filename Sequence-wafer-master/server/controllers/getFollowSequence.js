...
module.exports = async ctx => {
  ...
  var sequenceList = await mysql("Sequence").select("Sequence.*").innerJoin("UserSequenceMap", "Sequence.id", "UserSequenceMap.SequenceId").where({ userId: params.userId, joinStatus: 2 }).orderBy("Sequence.updatedAt", "desc").limit(pageSize).offset(pageSize * (page - 1))
