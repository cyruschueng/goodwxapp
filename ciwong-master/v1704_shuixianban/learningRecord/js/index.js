        ...
        methods:{
            ...
            urlTo:function(item){
                ...
                if(item.moduleId==124){  // online, in app h5 page
                    url = '#accessToken='+accessToken+'&clientId='+clientId+'&userId='+item.userId+'&brandId='+brandId+'&versionId='+item.versionId+'&doWorkId='+item.doWorkId+commentUrlParam+''
                    // localStorage.setItem('url',url)
                    location.href= HOST_project +　'onlinePaper/paperResult.html'+url;
                }else if(item.moduleId==15){ // listenspeak
                    if(item.doWorkPackageUrl.indexOf('http:')>-1){  // 旧资源
                        log = '开始请求JumpToOldListen_Speak'
                        // 跳转到app本地h5页面
                        ciwongPlugin.createEvent('JumpToOldListen_Speak' , [{source:3,params:item}] ,function( response ){
                    ...
                    // item.versionId = item.versionId && item.versionId!='0'? item.versionId : item.parentVersionId;
                    if(item.doWorkPackageUrl.indexOf('http:')>-1){  // 旧资源
                        log = '开始请求JumpToOldRead'
                        // 跳转到app本地h5页面
                        ciwongPlugin.createEvent('JumpToOldRead' , [{source:3,params:item}] ,function( response ){
