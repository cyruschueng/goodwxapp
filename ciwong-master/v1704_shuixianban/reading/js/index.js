...
function showPage(list,audioList){
    ...
    window.vue = new Vue({
        ...
        created:function(){
            ...
            // 评论内容传给App
            if(apiData.userInfo===false && apiData.answers.comment){
                log = 'app GetComments'
                ciwongPlugin && ciwongPlugin.createEvent('GetComments' , [{source:3,comment:apiData.answers.comment}] ,function( response ){
                    // alert(log + (typeof response=='string'?response:JSON.stringify(response)) )
                })
            }
            apiData.list.name && (document.title = apiData.list.name);
            // ios需要主动请求更改文档标题
            ...
            this.$nextTick(function(){
                ...
                // if(/SM/i.test(navigator.userAgent)){
                //     alert(1)
                // }
            })
        }
