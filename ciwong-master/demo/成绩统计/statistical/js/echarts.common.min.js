...
// echarts图标封装类
function echartsClass(id,option){
        ...
        myChart.setOption({
            ...
            series: [{
				...
				// markPoint:{
				// 	show:true,
				// 	symbol:'image://https://www.google.com.hk/images/branding/googlelogo/2x/googlelogo_color_120x44dp.png',
				// 	data:[{
				// 			name: '某个坐标',
				// 			coord: [10, 20]
				// 	}]
				// },
				symbol:'image://./images/chart-mark.png',
	            symbolSize:25,
				symbolOffset:[0,-20],
                data: option.data,
                label: {
		...
		myChart.on('datazoom', function (params) {
			if(params.batch[0].start==0 && params.batch[0].end==100){
				$.alert('已显示完全')
			}else if(params.batch[0].end==100){
				this.option.page>=this.option.totalPage?$.alert('已经到最后了'):this.option.callback();
			}else if(params.batch[0].start==0){
				$.alert('已经到最前面了')
			}
			console.log(params.batch[0].start,params)
			this._oldStart = params.batch[0].start;
		});
