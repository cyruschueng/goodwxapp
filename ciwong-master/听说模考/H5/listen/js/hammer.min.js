...
function hammerClass(element){
    if(!element) {alert('元素节点不能为空'); return;}
    ...
    var log,el,winWidth,winHeight,START_X,START_Y,currX,currY,scaleWidth,scaleHeight,ticking,transform,timer,mc,isInit;
    ...
    function init(){
    	isInit = true;
        $('.imgDialog').remove()
        // $('html,body').addClass('dialogOverflow')
        var html =  '<div class="imgDialog"><div>\
                        <div class="imgDialogBg"></div>\
                        <div class="imgDialogImgelement"><img id="imgDialog_img" src="'+element.src+'"></div>\
                    </div></div>';
        $('body').append(html)
        var winTop = $(window).scrollTop()
        $('.imgDialogImgelement').css('top',winTop+'px')
        $(window).on('touchmove',function(event){	event.preventDefault();	})
        // $('.imgDialog,.imgDialogImgelement,.imgDialogImgelement img').on('touchmove',function(event){	event.preventDefault();	});
        // $(window).scroll(function(event) {	$(window).scrollTop(winTop);	});

        log = {};
        el = document.querySelector('#imgDialog_img');
        winWidth = window.innerWidth;
        winHeight = window.innerHeight;
        START_X = Math.round((winWidth - el.offsetWidth) / 2);
        START_Y = Math.round((winHeight - el.offsetHeight) / 2);
        currX = START_X
        currY = START_Y
        scaleWidth = el.offsetWidth
        scaleHeight = el.offsetHeight
        ...
        */
        ...
        $('.video').css('left','-9999px')
        $('.imgDialog').fadeIn(300)
        resetElement();
        ...
        $('.imgDialog').click(function(){
            $('.imgDialog').hide()
        	$('.video').css('left','0')
            $('html,body').removeClass('dialogOverflow')
    ...
    function resetElement() {
        ...
        // 初始化缩放比例
        if(scale>1){
            var maxWidth = winWidth*4;   // 设置最大宽度
            var maxHeight = winHeight*4;   // 设置最大高度
            if(el.offsetWidth*scale>maxWidth) scale = maxWidth/el.offsetWidth
            if(el.offsetHeight*scale>maxHeight) scale = maxHeight/el.offsetHeight
        }else{
            scale = 1
        }
        scaleWidth = el.offsetWidth*scale;
        scaleHeight = el.offsetHeight*scale;
        // 缩放后的宽度大于屏幕宽度
        if(scaleWidth>winWidth){
            // 根据缩放比例初始化开始位置
            START_X = Math.round((scaleWidth - el.offsetWidth)/2);
            x = transform ? (transform.translate.x<START_X?transform.translate.x:START_X) : START_X;
            // 图片左侧不能小于屏幕边缘
            if(scaleWidth + x < winWidth+START_X) x =  winWidth - scaleWidth;
            // 图片右侧不能小于屏幕边缘
            if(scaleWidth + x - START_X < winWidth) x = -(scaleWidth - winWidth - START_X);
        }else{
            START_X = Math.round((winWidth - el.offsetWidth) / 2)
            if(transform && transform.translate.x<START_X && transform.translate.x+scaleWidth>winWidth){
        ...
        if(scaleHeight>winHeight){
            START_Y = Math.round((scaleHeight - el.offsetHeight) / 2);
            y = transform ? (transform.translate.y<START_Y?transform.translate.y:START_Y) : START_Y;
            // 图片上侧不能小于屏幕边缘
            if(scaleHeight + y < winHeight+START_Y) y =  winHeight - scaleHeight;
            // 图片下侧不能小于屏幕边缘
            if(scaleHeight + y - START_Y < winHeight) y = -(scaleHeight - winHeight - START_Y);
        }else{
            START_Y = Math.round((winHeight - el.offsetHeight) / 2)
            if(transform && transform.translate.y<START_Y && transform.translate.y+scaleHeight>winHeight){
    ...
    function onPan(ev) {
        var x,y;
        if(ev.type == 'panstart') {
            currX = transform.translate.x
            currY = transform.translate.y
            // event.preventDefault();
        }
        x = scaleWidth > winWidth ? currX + ev.deltaX : START_X;
        y = scaleHeight > winHeight ? currY + ev.deltaY : START_Y;
        el.className = '';
        transform.translate = {
            x: x,
            y: y
