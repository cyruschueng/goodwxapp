...
//查询个人详情
function personaInfo(that, uid, userData, display, phone,mer,show,sort){
  var str = {
    "OperationType": "10020",
    "uid": wx.getStorageSync("uid")
  }
  ...
  wx.request({
    ...
    success: function (res) {
      ...
      if (res.data.ID) {
        ...
        if (res.data.sort == "account") {
          that.setData({
            sort: "会员"
          })
        } else if (res.data.sort == "two_sdl") {
          that.setData({
            sort: "初级代理"
          })
        } else if (res.data.sort == "one_sdl") {
          that.setData({
            sort: "高级代理"
          })
        } else if (res.data.sort == "two_jms") {
          that.setData({
            sort: "初级加盟商"
          })
        } else if (res.data.sort == "one_jms") {
          that.setData({
            sort: "高级加盟商"
          })
        } else if (res.data.sort == "thr_shr") {
          that.setData({
            sort: "初级合伙人"
          })
        } else if (res.data.sort == "two_shr") {
          that.setData({
            sort: "中级合伙人"
          })
        } else if (res.data.sort == "one_shr") {
          that.setData({
            sort: "高级合伙人"
          })
        }
        wx.setStorageSync("sort", that.data.sort)
      }
    }
  })
}
