...
var app = getApp();
//上拉加载更多
var loadMore = function (that, type) {
  console.log("账单列表")
  that.setData({
    hidden: true
  });
        ...
        if (that.data.page == 1) {
          if (res.data.data.length) {
            ...
            for (var i = 0; i < data.length; i++) {
              ...
              var str = data[i].bankCard;
              var length = data[i].bankCard.length;
              var reg = /^(\d{4})\d+(\d{4})$/;
              str = str.replace(reg, "$1****$2");
              card.push(str)
              data[i].bankCard = data[i].bankCard.slice(length-4,length)
            }
            console.log("slice")
            console.log(res.data.data[0].createTime.split("/")[2].split(" ")[0])
            //缓存订单数据
            wx.setStorageSync("detailOrder", data);//时间排序订单详情
            that.setData({
              status: arr,
              cardNum: card,
              detailOrder: data,
...
Page({
  ...
  onTab: function () {
    ...
    var type = that.data.className ? "1" : "0";
    //从新点击之后，page归1
    that.setData({
      page: 1
    })
