...
Page({
    data: {
        ifPublish: false,
        placeholder: '请输入问题内容，预计5分钟内即可收到回答。48小时内无人抢答，将会全额退款',
        picPath: [],
        textAreaVal: "",
        textLength: 0,
        recordingTime: "0:00",
    ...
    delayChangeInputState: function () {
        var This = this;
        setTimeout(function () {
    ...
    changeInputState: function () {
        ...
        if (!ifKeyboard && (this.data.ifRecording || this.data.ifFinishRecord)) {
            ...
            mta.Event.stat("publishchangesound",{})
            wx.showModal({
                title: '温馨提示',
                content: '是否放弃已输入内容',
                success: function (res) {
    ...
    playRecordEv: function () {
        ...
        if (this.data.playVoice) {
            ...
            this.setData({
                playVoice: false,
                playIcon: '../../../images/listening.gif'
            });
            let time = parseInt(this.formatToSeconds(this.data.recordingTime)) * 1000;
            setTimeout(() => {
                this.setData({
                    playIcon: '../../../images/sound.png'
                })
    ...
    delayPublishEv: function (ev) {
        mta.Event.stat("publishclickbtn",{})
        this.setData({
    ...
    publishEv: function (ev) {
        ...
        if (!this.data.ifPublish) {
            ...
            //setTimeout(function () {
                ...
                if (helpType == 2) {
                    data = { content, helpType, sid, web, formId }
                } else {
                    data = { voiceUrl, helpType, sid, web, formId }
                }
                if (resourcesArr.length > 0) {
                    let resources = resourcesArr.join(",");
                    if (helpType == 2) {
                        data = { content, helpType, resources, sid, web, formId }
                    } else {
                        data = { voiceUrl, helpType, resources, sid, web, formId }
                    }
                }
                if (This.data.ifRecording) {
                    wx.showModal({
    ...
    uploadPic: function () {
        ...
        if (this.data.ifRecording) {
            ...
            wx.chooseImage({
                ...
                sizeType: ['original'],
                success: function (res) {
                    mta.Event.stat("publishaddpicsuc",{})
    ...
    showRule() {
        mta.Event.stat("publishclickrules",{});
