    ...
    <div class="article">
        ...
        <div class="article__ft">
            ...
            <div class="thumbsUpList" v-if="ThumbsUpUser.length!==0">
                <div class="item" v-for="(item, index) in ThumbsUpUser">
        ...
        <div class="utils">
            <div class="thumbsUpBox">
                ...
                <div style="width: 100%;margin-top: 5px;">
                    ...
                    <box gap="15px" v-if="!this.loadding && (this.pageNo >= this.pageCount - 1)">
        ...
        methods: {
            ajaxLoadArticleDetail() {
                return new Promise((resolve, reject) => {
                    this.$axios.post({
                        url: '/cust/article/services/loadArticleDetail',
                        data: {
                        ...
                        if (result.thumbsupcount > result.ThumbsUpUser.length) {
                            ...
                            for (let i = 0; i < count; i++) {
                                this.ThumbsUpUser.push({headurl: `/static/cust/article/img/userImg/${userImg[i]}.jpg`})
                            }
                        }
                    })
                })
            ...
            ajaxSubmitArticleComment() {
                this.$axios.post({
                    url: '/cust/article/services/submitArticleComment',
                    data: {
                        articleid: this.$route.query.articleid,
            ...
            ajaxLoadArticleCommentList(append = true) {
                ...
                return new Promise((resolve, reject) => {
                    this.$axios.post({
                        url: '/cust/article/services/loadArticleCommentList',
                        data: {
                            articleid: this.$route.query.articleid,
                            pageNo: this.pageNo,
            ...
            ajaxCallBackArticleShared(type) {
                this.$axios.post({
                    url: '/cust/article/services/callBackArticleShared',
                    data: {
                        articleid: this.$route.query.articleid,
                        sharetype: type,
        ...
        async created() {
            ...
            this.$wechat.ready(() => {
                ...
                this.weixinShare({
                    ...
                    link: `${window.location.protocol}//${window.location.host}/cust/article/index?articleid=${this.$route.query.articleid}&fpuserid=${this.$route.query.fpuserid}&shareuserid=${result.shareuserid}&level=${parseInt(this.$route.query.level) + 1}#/?articleid=${this.$route.query.articleid}&fpuserid=${this.$route.query.fpuserid}&shareuserid=${result.shareuserid}&level=${parseInt(this.$route.query.level) + 1}`, // 分享链接，该链接域名或路径必须与当前页面对应的公众号JS安全域名一致
