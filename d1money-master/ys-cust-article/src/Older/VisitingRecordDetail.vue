...
<template>
    <div class="visitingRecordDetail">
...
<script>
    ...
    export default {
        ...
        methods: {
            // 分页加载团队拜访记录
            ajaxVisitingRecordDetail() {
                this.$axios.post({
                    url: 'visitingRecordDetail',
                    data: {
                        visitId: this.visitId
                    }
                }).then(result => {
                    this.customInfo.userName = result[0].name
                    this.customInfo.customName = result[0].userName
                    this.customInfo.avatar = result[0].avatar
                    this.list.push(result[0])
                }).catch(error => {
        ...
        async mounted() {
            this.$refs.dmscroller.finishInfinite()
        },
        destroyed() {
        }
    }
