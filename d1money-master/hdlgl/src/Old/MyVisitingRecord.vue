                    ...
                    <template v-for="(group, group_index) in visitTimeList.list">
                        <group class="visitTime__group" :title="time(group.date) +' ('+group.list[0].dayCount+'次实地拜访)'">
                            <template v-for="(item, index) in group.list">
                                <cell
                                    @click.native="goto('CustomDetail',{ customerId: item.customerId })"
                                >
                                    ...
                                    <div class="visitTime__content" slot="inline-desc">
                                        <p class="desc">
                                            <template v-if="item.type === 1">【补充拜访记录】</template>{{item.desc}}
                                        </p>
...
//                let customListScrollTop = this.$el.querySelector('.customList').scrollTop
            ...
            async infinite(done) {
                ...
                let newListTemp = await this.ajaxPageVisitingRecordsByCurrentUser()
            ...
            ajaxPageVisitingRecordsByCurrentUser() {
                return new Promise((resolve, reject) => {
                    this.$http.post('/ys/gzrz/services/pageVisitingRecordsByCurrentUser', querystring.stringify({
...
//            客户列表 ajax
            ajaxLoadAllCustomersByCurrentUser() {
                return new Promise((resolve, reject) => {
                    this.$http.post('/ys/gzrz/services/loadAllCustomersByCurrentUser')
                        .then(result => {
                            if (result.data.code === 'SUCCESS') {
                                resolve(result.data.body)
                            } else {
