            ...
            <div v-if="commentList && commentList.length!=0" :class="['comments']">
                ...
                <template v-for="(comment, commentIndex) in commentList">
                    <div class="comment-item comment" @click.stop="reply(index, commentIndex)">
                        <span class="user">{{comment.name}}:</span> {{comment.comment}}
                    </div>
                    <!--回复-->
                    <template v-for="(reply, index) in comment.commentReplylist">
                        <div class="comment-item reply">
                            <span class="user">{{reply.name}}</span>回复<span class="user" style="margin-right:0;margin-left:.5em">{{comment.name}}</span>
                            {{reply.replyComment}}
                        </div>
                    </template>
                </template>
...
<script>
    ...
    export default {
        props: ['id', 'thumbList', 'commentList', 'isThumbsUp', 'dateTime', 'index'],
        data() {
            return {}
        },
        mounted() {
