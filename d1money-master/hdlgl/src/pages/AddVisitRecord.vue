    ...
    <div v-cloak class="addVisitTime">
        <div class="addVisitTime__bd">
            ...
            <group class="customName" :gutter="0">
                <x-input v-model="fromValus.customerName"
                         ...
                         @focus.native="onFocusByCustomName()"
                         @blur.native="onBlurByCustomName()"
                ></x-input>
            </group>
            ...
            <p class="position" style="padding-left: 8px; color: #666666;">
                <i class="iconfont icon-ai-seat"></i>{{fromValus.location}}
            </p>
        ...
        <div class="soundRecordingDialog" v-transfer-dom>
            <confirm v-model="soundRecordingDialogIsShow"
                ...
                <!--<i class="iconfont icon-zhongxinluyin" @clic="reSoundRecording"></i>-->
            </confirm>
        </div>
        <!--<button @click="uploadVoice">上传语音</button>-->
...
//            提交
            ajaxFrom() {
                ...
                let location = this.fromValus.location
                if (location === '正在获取位置...') {
                    location = '【未知位置】'
                }
                this.$axios.post({
                    url: 'doSubmitVisitingRecord',
                    data: {
                        isLeader: this.isLeader,
                        customerId: this.fromValus.customerId,
                        customerName: this.fromValus.customerName,
                        location: location,
                        longitude: this.fromValus.longitude,
                        latitude: this.fromValus.latitude,
...
//            微信定位 提交
        ...
        mounted() {
            this.$wechat.ready(() => {
                ...
                this.getLocation()
                    .then(lonAndLat => {
                        this.fromValus.longitude = lonAndLat.longitude
                        this.fromValus.latitude = lonAndLat.latitude
                        this.ajaxLonlatToLocation(this.fromValus.longitude, this.fromValus.latitude)
                            .then(res => {
                                this.fromValus.location = res
                            })
                            .catch(() => {
                                this.$vux.toast.show({
                                    type: 'cancel',
