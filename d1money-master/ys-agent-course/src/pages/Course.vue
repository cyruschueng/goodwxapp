                ...
                <template v-for="(item, index) in list">
                    <flexbox-item class="courseItem" :span="1/2">
                        <course-cell
                            ...
                            :desc="item.introduce"
                            :img="item.imgsrc"
                            :learnNumber="item.learncount"
                            :mask=".6"
                            :isRadius="true"
                            @click.native="routerLink('CourseCatalog',{curriculumid: item.id})">
                        </course-cell>
                    </flexbox-item>
                </template>
            </flexbox>
        ...
        methods: {
            ...
            ajaxLoadAllPosterType() {
                return new Promise((resolve, reject) => {
                    this.$axios.post({
                        url: '/agent/curriculum/services/loadCurriculumType',
                        tips: true
                    }).then(result => {
                        result.unshift({
                            id: -1,
            ...
            ajaxPageDateByList() {
                return new Promise((resolve, reject) => {
                    this.$axios.post({
                        url: '/agent/curriculum/services/loadCurriculumList',
                        data: {
                            typeid: this.typeid,
                            pageNo: this.pageNo,
