...
<template>
    <div class="productDetail">
        ...
        <template v-else="loading">
            <!--头部-->
            <div class="productDetail__hd">
                <masker class="productDetail__hd__masker" :opacity="0.7">
                    <div slot="default" class="bgImg">
                        <img :src="productDetail && productDetail.bigimgsrc">
                    </div>
            ...
            <!--尾部-->
            <div class="productDetail__ft">
                <x-button :gradients="['#1D62F0', '#19D5FD']"
                          :link="{name: 'ProductIframe', params:{id: this.id}}">去邀约
                </x-button>
            </div>
        </template>
    </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            async init() {
                try {
                    let datas = await this.ajaxLoadDetail()
                    ...
                    this.$wechat.ready(() => {
                        ...
                        this.weixinShare({
                            ...
                            link: `${window.location.protocol}//${window.location.host}/cust/product/index?id=${this.id}&shareuserid=${this.user.id}&fpuserid=${this.user.id}&level=2#/?id=${this.id}&shareuserid=${this.user.id}&fpuserid=${this.user.id}&level=2`, // 分享链接，该链接域名或路径必须与当前页面对应的公众号JS安全域名一致
            ...
            ajaxLoadDetail() {
                return new Promise((resolve, reject) => {
                    this.$axios.post({
                        url: '/agent/product/services/loadDetail',
                        data: {
                            productid: this.id,
                            type: 0
                        }
                        ...
                        this.tabs[0].num = res.totalbuytcount
            ...
            lookDesc() {
                this.$vux.alert.show({
                    ...
                    content: this.productDetail.introduce
                })
            },
            /***
             * 分享回调接口
            ...
            ajaxCallBackShared(type) {
                this.$axios.post({
                    url: '/agent/product/services/sharecallBack',
                    data: {
                        productid: this.id,
                        sharetype: type,
            ...
            ajaxPageDateByList() {
                return new Promise((resolve, reject) => {
                    this.$axios.post({
                        url: '/agent/product/services/loaduserdataByid',
                        data: {
                            productid: this.id,
