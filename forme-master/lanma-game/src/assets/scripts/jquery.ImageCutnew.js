...
(function($){
	$.fn.imageCut = function(options){
		...
		_maskimage_.crossOrigin = "Anonymous";
		...
		image.onload = function(){
			...
			if(box.html()!=''){
				...
				cutMask = document.getElementById('cut-mask');
			...
			function compress(source_img,quality){
				...
				if(source_img.width>source_img.height){
					imageWidth=MaxW;
					var imageHeight=Math.round(MaxH*(source_img.height/source_img.width));
				}else if(source_img.width<source_img.height){
					var imageHeight=MaxH;
					var imageWidth=Math.round(MaxW*(source_img.width/source_img.height));
				}else{
					var imageWidth=MaxW;
					var imageHeight=MaxH;
				}
