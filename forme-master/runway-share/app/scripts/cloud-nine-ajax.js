...
// Attach a bunch of functions for handling common AJAX events
  jQuery.each( [
    ...
    jQuery.fn[ type ] = function( fn ) {
      return jQuery(document).on( type, fn );
    };
  } );
...
//  }
  ...
  /** cloudNineAjax start **/
  if (window.cloudNineAjax) {
    return console.error('cloudNineAjax Exit');
  }

  window.cloudNineAjax = {};

  jQuery.each( [ 'get', 'post' ], function( i, method ) {
    window.cloudNineAjax[ method ] = function( url, data, callback, type ) {
  ...
  window.cloudNineAjax['ajax'] = function(url, options) {
    // If url is an object, simulate pre-1.5 signature
    if ( typeof url === 'object' ) {
      options = url;
      url = undefined;
  ...
  window.cloudNineAjax['ajaxStart'] = jQuery(document).ajaxStart;
  window.cloudNineAjax['ajaxComplete'] = jQuery(document).ajaxComplete;
  /** cloudNineAjax end **/
