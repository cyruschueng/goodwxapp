...
(function (jq) {
    ...
    //旋转动画
    window.setRotateAnimation = function (view, fromRotateY, toRotateY, duration, callback, repeatCount) {
        var anim = new RotateAnimation();
        anim.fromRotateY = fromRotateY;
        anim.toRotateY = toRotateY;
        anim.duration = duration;
    ...
    window.httpPost = function (url, data, callback,callbackfail) {
    ...
    window.httpPostTwo=function (url,data,callback) {
        jq.ajax({
            type: "post",
            url: url,
            data: data,
    ...
    //         data: obj.data||{},
    //         headers: {
    //             'Cookie': 'JSESSIONID=77519B4BA3D706BFE6622A7DE6FA3185'
    //         },
    ...
    util.clear = function (arr) {
        arr.splice(0, arr.length);
    };
    util.addAll = function (source, target) {
        target.forEach(function (item) {
            source.push(item);
        })
    };
    util.hideAnimation = function (view, callback) {
        var anim = new AlphaAnimation();
        anim.duration = 200;
        anim.fromAlpha = 1;
        anim.toAlpha = 0;
    ...
    util.showAnimation = function (view, callback) {
        var anim = new AlphaAnimation();
        anim.duration = 300;
        anim.fromAlpha = 0;
        anim.toAlpha = 1;
    ...
    util.switchAnimation = function (view, hideCallback, showCallback) {
        var self = this;
        this.hideAnimation(view, function () {
            hideCallback();
            self.showAnimation(view, showCallback);
        })
    };
    ...
    util.commonchoose = function (par) {
        if (!par || !par.subViews || par.subViews.length < 1) {
            return
        }
        ;
    ...
    util.getYoutuThumbnail = function(url){
        if(url && url.replace){
            // http://material-10002033.file.myqcloud.com/pyeongchang2018/userimg/8ec793c9b1df46e2a7fb846f2c655ad5.png
            // http://cimg2.visualbusiness.cn/pyeongchang2018/userimg/8ec793c9b1df46e2a7fb846f2c655ad5.png/thumbnail
            url = url.replace('material-10002033.file.myqcloud.com', 'cimg2.visualbusiness.cn')
            url += '/thumbnail'
