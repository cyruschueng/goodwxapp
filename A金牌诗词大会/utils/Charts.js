...
function getRadarDataPoints(angleList, center, radius, series, opts) {
    ...
    var maxDataArray = radarOption.maxDataArray || [];
    var data = [];
    series.forEach(function (each) {
        ...
        each.data.forEach(function (item, index) {
            ...
            //不同维度的Max值不一样
            if (maxDataArray.length) {
                maxData = maxDataArray[index];
            }
...
function drawRadarLabel(angleList, radius, centerPosition, opts, config, context) {
    var radarOption = opts.extra.radar || {};
    var categoriesOffset = opts.categoriesOffset || {};
    ...
    angleList.forEach(function (angle, index) {
        ...
        // 没时间去找标签位置的实现过程  先这么处理吧调整标签的位置
        var offset = categoriesOffset[index];
        if (offset) {
            startX += offset.X || 0;
            startY += offset.Y || 0;
        }
        context.fillText(opts.categories[index] || '', startX, startY + ((radarOption.labelFontSize || config.fontSize) / 2));
    });
    context.stroke();
    context.closePath();
}
...
function drawRadarDataPoints(series, opts, config, context) {
    ...
    var centerPosition = {
        x: (opts.width + config.radarLabelTextMargin) / 2,
        y: (opts.height - config.legendHeight) / 2
    };
    ...
    context.draw(false, function () {
        wx.canvasToTempFilePath({
            ...
            success: function(res) {
                context.canvasImgPath = res.tempFilePath;
            } 
        });
    });
}
