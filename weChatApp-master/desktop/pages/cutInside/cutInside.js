            ...
            if(pic){
              ...
              switch (parseInt(source)) {
                case 1: api.bianLian(pic.url, model, function(res) {
                  wx.setStorageSync("remoteImage",pic.url);
                  wx.hideNavigationBarLoading();
                  wx.hideToast();
                  util.downloadAndPreview(res.url,'这个可以换脸的小程序太好玩了','/pages/zhuangx/list/list?type=2','生成中...')
                ...
                case 3: api.cosmetic(pic.url, model, function(res) {
                  ...
                  console.log("showLoading",app.globalData.showLoading)
                  app.globalData.showLoading = false;
                  wx.hideNavigationBarLoading();
                  wx.hideToast();
                  that.showZanToast(res);
