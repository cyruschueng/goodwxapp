...
Page(Object.assign({}, Zan.Toast, {
  ...
  switchChange:function (e) {
    ...
    selectPic:function (e) {
        ...
        wx.chooseImage({
            ...
            success: function (res) {
                that.afterPicChoose(res.tempFilePaths[0])
            }
        })
    },
    afterPicChoose: function (localPic) {
        var that = this;

        that.data[app][type].list[index].pic = {"url":'http://icons.maiyizhi.cn/uploading.png',"width":300,"height":300}
        that.setData({
            app: that.data[app]
        });
        ...
        upload.uploadSingleB({path: localPic, state: 1}, function (pic) {
            if(pic){
                that.data[app][type].list[index].pic = {"url":pic.url,"width":pic.width,"height":pic.height}
                that.setData({
                    app: that.data[app]
                });
                wx.setStorageSync(app, that.data[app]);
    ...
    formSubmit: function (e) {
        ...
        if(utils.isEmptyObject(_app[type].list[index].pic)){
            this.showZanToast('必须选择图片呢');
            return
        }
        this.saveData()
        wx.navigateBack();
