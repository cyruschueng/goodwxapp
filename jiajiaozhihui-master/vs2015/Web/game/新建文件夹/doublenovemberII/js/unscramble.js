...
Unscramble.getData = function () {
    $.ajax({
        url: "/game/doublenovemberII/server/unscramble.ashx",
        type: "POST",
        dataType: "JSON",
        data: { 'method': 'get' },
        beforeSend: function () {
...
var menu_item = function (book,data) {
    ...
    $.each(data, function (index, context) {
        html += "<div class='col-xs-3 text-center' style='padding-top:3px; padding-bottom:3px;'><a style=' color:#fff; ' href='unscramble.aspx?book=" + book + "&index=" + context.index + "&weixinid=" + $("#hfWeixinID").val() + "&openid=" + $("#hfOpenID").val() + "'>第" + context.index + "页</a></div>";
    })
    return html;
}
