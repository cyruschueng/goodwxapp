...
Additioninfo.Submit = function () {
    ...
    $.ajax({
        url: "/game/doublenovemberII/server/additioninfo.ashx",
        type: "POST",
        dataType: "json",
        beforeSend: function () {
            ...
            if (!$.isEmptyObject(data)) {
                if (data.code == 0) {
                    if ($("#hfFrom").val() == "normal") {
                        window.location.href = "/game/doublenovemberII/info-user.aspx?openid=" + $("#hfOpenID").val() + "&id=76";
                    } else {
                        window.location.href = "/game/doublenovemberII/partin.aspx?openid=" + $("#hfOpenID").val() + "&id=76";
                    }
                }
            }
        },
