    ...
    public class partin : IHttpHandler
    {
        ...
        private void wc_DownloadFileCompleted(object sender, AsyncCompletedEventArgs e)
        {
            ...
            if (entity["code"].ToString() == "0")
            {
                ...
                WXHelper.WriteNode("上传腾讯云服务器失败：" + entity["code"].ToString() + entity["message"].ToString() + " openid=" + FolderName + " MyMediaID=" + MyMediaID + "; 路径：" + LocalPath, "yun.txt");
            }
        }
        private void AddFile(string imgUrl, string fileid, string resume, string owner,string bookName,string pageNumber)
        {
            ...
            try
            {
                ...
                ResultMsg = "{\"code\":\"-1\",\"message\":\"" + ex.Message + "\"}";
                WXHelper.WriteNode("game/doublenovemberII/server/partin.ashx(FolderName:" + FolderName + ")" + ex.Message);
            }
            try
            {
