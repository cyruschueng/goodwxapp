...
namespace SfSoft.web.game.doublenovemberII.server
{
    ...
    /// friend_works 的摘要说明
    /// </summary>
    public class friend_works : IHttpHandler
    {
        ...
        private string GetData(HttpContext context)
        {
            ...
            try
            {
                string openid = context.Request["openid"].ToString().ConvertBase64TocChars();//我的朋友
                openid = DEncrypt.Decrypt(openid, WXConfig.EncryptKey);
                string like = context.Request["like"].ToString().ConvertBase64TocChars();//点赞的人，是经加密码的
                like = DEncrypt.Decrypt(like, WXConfig.EncryptKey);
                ...
                sb.Append("select top " + pagesize + " *  from (")
                    ...
                    .Append(" case  when exists(select top 1 * from WX_Doublenovember_Like where From_OpenID='" + like + "' and FileID=a.ID) then 1")
