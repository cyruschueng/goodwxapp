...
namespace SfSoft.web.game.qzsf.start
{
    public partial class invite : System.Web.UI.Page
    {
                ...
                */
                ...
                if (baseOauth.OAuthResult.Code == OAuthCodeEnum.error.ToString())
                {
                    if (baseOauth.OAuthResult.Msg.Contains("api unauthorized"))
                    {
                        ...
                        string url = ShenerWeiXin.WXConfig.AuthURL + "game/qzsf/friend-invite.aspx";
                        Response.Redirect(url);
                    }
                    else
                    {
                        baseOauth.RedirectUrl = FormatUrl(ShenerWeiXin.WXConfig.AuthURL + "game/qzsf/start/invite.aspx");
                        baseOauth.Redirect();
                    }
                }
                else
                {
                    ...
                    if (model != null)
                    {
                        string url = FormatUrl(ShenerWeiXin.WXConfig.AuthURL + "game/qzsf/friend-invite.aspx?mode=database&id=76");
                        Response.Redirect(url);
                    }
                    else
                    {
                        ...
                        {
                            string url = FormatUrl(ShenerWeiXin.WXConfig.AuthURL + "game/qzsf/friend-invite.aspx?mode=reflesh&id=76");
                            Response.Redirect(url);
                        }
                        else
                        {
        ...
        private string FormatUrl(string url)
        {
            ...
            {
                string parameter = Request.QueryString["menu"];
                int index = url.LastIndexOf("?");
                if (index > -1)
                {
                    url += "&f=" + parameter;
