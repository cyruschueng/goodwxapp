...
{
    public partial class infoworks : System.Web.UI.Page
    {
                ...
                */
                ...
                if (baseOauth.OAuthResult.Code == OAuthCodeEnum.error.ToString())
                {
                    if (baseOauth.OAuthResult.Msg.Contains("api unauthorized"))
                    {
                        ...
                        string url = ShenerWeiXin.WXConfig.AuthURL + "game/qzsf/info-works.aspx";
                        Response.Redirect(url);
                    }
                    else
                    {
                        baseOauth.RedirectUrl = FormatUrl(ShenerWeiXin.WXConfig.AuthURL + "game/qzsf/start/infoworks.aspx");
                        baseOauth.Redirect();
                    }
                }
                else
                {
                    ...
                    if (model != null)
                    {
                        string url = FormatUrl(ShenerWeiXin.WXConfig.AuthURL + "game/qzsf/info-works.aspx?mode=database&id=76");
                        Response.Redirect(url);
                    }
                    else
                    {
                        ...
                        {
                            string url = FormatUrl(ShenerWeiXin.WXConfig.AuthURL + "game/qzsf/info-works.aspx?mode=reflesh&id=76");
                            Response.Redirect(url);
                        }
                        else
                        {
