...
namespace SfSoft.web.game.qzsf
{
    public class Diamond
    {
        ...
        private void UpdateAward(AwardModel awardModel)
        {
            ...
            if (model != null)
            {
                if (model.DiamondEarn != null)
                {
                    model.DiamondEarn += awardModel.Min;
                }
                else
                {
                    model.DiamondEarn = awardModel.Min;
                }
                WXHelper.WriteNode("DiamondEarn"+(model.DiamondEarn ?? 0).ToString(), "share.txt");
                bll.Update(model);
            }
            else
            {
        ...
        private void UpdateAwardDetail(AwardModel awardModel)
        {
            ...
            model.Remark = enumDiamond.ToString() + "获取" + EnumAwardType.钻石.ToString();
            bll.Add(model);
        }
