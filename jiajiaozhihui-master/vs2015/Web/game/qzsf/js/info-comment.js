$(function () {
    ...
    $("#btnDelete").click(function () {
        var id = deleteDialog.getDeleteId();
        deleteDialog.hide();
        subDeleteFile(id);
    });
    ...
    if (!$.isEmptyObject(data.comment.ds)) {
        $.each(data.comment.ds, function (index, context) {
            ...
            if (context.FileID == id) {
                if ($("#hfUserID").val() == context.OpenID) {
                    html += "   <li style=' padding:3px 5px' id='mycomment"+context.ID+"'><span style='color:#0034FF'>" + context.NickName + "</span>：" + context.Content + "&nbsp;&nbsp;<button onclick='deleteComment("+context.ID+")'  type='button' class='btn btn-xs btn-default  ' ><span class='glyphicon glyphicon-remove-sign'></span></button></li>";
                } else {
