    ...
    public class index : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            switch (method)
            {
                ...
                case "location":
                    ...
                    break;
                case "gettop":
                    result = GetTop(context);
                    break;
                case "inserttop":
                    result = InsertTop(context);
                        break;
                case "replacetop":
                        result = ReplaceTop(context);
                        break;
            }
            context.Response.Write(result);
        }

        private string ReplaceTop(HttpContext context)
        {
            return "{\"code\":\"0\",\"msg\":\"ok\"}";
        }

        private string InsertTop(HttpContext context)
        {
            return "{\"code\":\"0\",\"msg\":\"ok\"}";
        }

        private string GetTop(HttpContext context)
        {
            return "{\"code\":\"0\",\"msg\":\"ok\",\"mode\":\"replace\"}";
        }
        ...
        private string GetData(HttpContext context)
        {
            ...
            try
            {
                ...
                sb.Append("select top " + pagesize + " *  from (")
                    ...
                    .Append(" case ")
                    .Append(" when exists(select * from WX_Doublenovember_Invite where Status=1 and (FromOpenID=a.OpenID or ToOpenID=a.OpenID ) and (FromOpenID='" + openid + "' or ToOpenID='" + openid + "')) then '1'")
                ...
                if (ds != null && ds.Tables[0] != null && ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    string adv = GetAdvertisement();
                    result = result.Insert(result.Length - 1, ",\"comment\":" + s);
                    result = result.Insert(result.Length - 1, ",\"adv\":" + adv);
                }
                return result;
            }
            catch (Exception ex)
        ...
        private string GetComment(DataSet data)
        {
            ...
            if (ids.Length != 0)
            {
                ...
                sb.Append(" left join (select * from dbo.WX_Items_User where ItemsId=1) b on a.openid=b.openid");
        ...
        private string GetAdvertisement()
        {
            ...
            string sql = "Select ID,Name,ImgUrl,MediaUrl,OuterLink,Positions,CreateDate,Resume,datalength(TextContent) as ContentLength From WX_Advertisement where Own='emc.activity.double11' and isnull(IsAct,0)=1 ";
        ...
        private string Top(HttpContext context)
        {
            ...
            {
                id = context.Request["fileid"].ToString();
            }
            if (id != "")
            {
                ...
                if (model != null && ((model.IsTop??0)==0))
                {
                    model.IsTop = 1;
                    bll.Update(model);
                    result = "{\"code\":\"0\",\"msg\":\"提交成功\"}";
                }
                else if (model != null && model.IsTop==1)
                {
                    model.IsTop = 0;
                    bll.Update(model);
                    result = "{\"code\":\"0\",\"msg\":\"提交成功\"}";
        ...
        private string Location(HttpContext context)
        {
            string result = "{}";
            try
            {
                ...
                ShenerWeiXin.Common.Location.BaiDuApi api = new ShenerWeiXin.Common.Location.BaiDuApi(openid, latitude, longitude);
