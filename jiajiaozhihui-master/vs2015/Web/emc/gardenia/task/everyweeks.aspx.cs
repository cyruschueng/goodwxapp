...
{
    public partial class everyweeks : SfSoft.SfEmc.EmcBasePage
    {
        ...
        private void BindData()
        {
            ...
            {
                courseList = courseBll.GetModelList("Theme=" + ddlTheme.SelectedValue + " and CourseType=" + _config["courses"]["coursesType"].ToString());
            }
            else
            {
                courseList = courseBll.GetModelList("");
            }
        ...
        {
            BLL.wx_gardenia_task bll = new BLL.wx_gardenia_task();
            var list = bll.GetModelList("class_id=" + _classId + " and data_type='everyweek'");
        ...
        protected void GridView3_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                {
                    var x = e.Row.Cells[5];
                    var c= x.Controls[2] as LinkButton;
                    LinkButton temp = e.Row.Cells[5].Controls[2] as LinkButton;
                    temp.Attributes.Add("onclick", "javascript:return confirm('你确认要删除：\"" + e.Row.Cells[2].Text + "\"吗?')");
                }
                if (e.Row.RowState == DataControlRowState.Edit) {

                    TextBox txtStartUpDate = (TextBox)e.Row.Cells[4].FindControl("txtStartUpDate");
