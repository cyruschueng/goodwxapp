...
Page({
  data:{
    ...
    items:[],
    userInfo:{},
    habitId:app.globalData.habitId,
    showComment:false,
    showReply:false,
    comment:{
      detailId:0,
  ...
  getdetail:function(){
    ...
    wx.request({
      url: host+"/api/habit/detail/"+this.data.habitId+"/"+startIndex+"/"+endIndex+"/"+sessionid,
      method: 'GET',
      success: function(res) {
        ...
        if(res.data.success==true){
          that.setData({
            items:that.updateSourse(res.data.list),
  ...
  updateLike:function(e){
    ...
    var host=app.globalData.host;
    var list= this.data.items[index].like.list;
  ...
  addLike:function(index){
    ...
    var m=this.data.items[index].like.list;
    m.push(headImgUrl)
    this.setData({
      ['items['+index+'].like.list']:m,
      ['items['+index+'].like.isLike']:true,
    })
  },
  removeLike:function(index){
    ...
    this.setData({
      ['items['+index+'].like.list']:m,
      ['items['+index+'].like.isLike']:false,
    })
  },
  showComment:function(e){
    console.log(e);
    if(this.data.showComment==false){
      this.setData({
        showComment:true,
        comment:{
