...
namespace SfSoft.web.AppStart.Audio
{
    /// <summary>
    /// Baseinfo 的摘要说明
    /// </summary>
        ...
        private void GetOpenidAndAccessToken(HttpContext context)
        {
            ...
            else
            {
                ...
                string url = string.Format("https://open.weixin.qq.com/connect/oauth2/authorize?appid={0}&redirect_uri={1}&response_type=code&scope=snsapi_base&state={2}#wechat_redirect",
                    WxBaseConfig.AppID,
                    WxBaseConfig.AudioRedirectBaseInfoUrl + "?r=" + redirect_url + "@" + share,
        ...
        private void GetOpenidAndAccessTokenFromCode(string code)
        {
            ...
            try
            {
                ...
                if (userInfoProvide.ExistsOpenId(jsonResult["openid"].ToString()) == false)
                {
                    ...
                    WxBaseConfig.AppID,
                    WxBaseConfig.AudioRedirectUserInfoUrl + "?r=" + redirect_url + "@" + share,
                    state);
                ...
                else
                {
                    ...
                    {
                        HttpContext.Current.Response.Redirect(redirect_url + "#/" + hash + "?o=" + HttpContext.Current.Server.UrlEncode(o) + "&m=" + HttpContext.Current.Server.UrlEncode(m) + "&t=" + GetTimeStamp());
                    }
                }
            }
            catch (Exception ex)
