...
app.factory('audiosService',['$http','$q','ngAudio',function($http,$q,ngAudio){
    var _songRemember=[];
    var _currUrl='';
    var _audio;
    var _load=function(path){
        if (_songRemember[path]) {
            _audio = _songRemember[path];
        } else {
            _audio = ngAudio.load(path);
            _audio.volume = 0.8;
            _songRemember[path] = _audio;
        }
    };
    var _play=function(path){
        if(_currUrl!=path){
            for(var audio in _songRemember){
                if(audio.played) audio.stop();
            }
        }else{
            _currUrl=path;
        }
        if(_songRemember[path]){
            var audio = _songRemember[path];
            audio.play();
        }
    };
    var _stop=function(path){
        if(_songRemember[path]){
            var audio = _songRemember[path];
            audio.stop();
        }
