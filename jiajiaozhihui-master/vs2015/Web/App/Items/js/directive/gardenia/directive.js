...
app.directive('checkMemberAudio',[function(){
    return {
        ...
        link: function (scope, ele, attr,ctr) {
            ...
            var index=attr.audioIndex;
            ele.on('click',function(){
                if(state!=1){
                    scope.audios[index].stop();
                    ctr.check(state);
                }else{
...
app.directive('checkMemberCoursesAudio',[function(){
    return {
        restrict: 'EA',
        require:'^memberAction',
        link: function (scope, ele, attr,ctr) {
            var state=attr.checkMemberCoursesAudio;
            ele.on('click',function(){
                if(state!=1){
                    scope.audio.stop();
                    ctr.check(state);
