$(function () {
    ...
    function init() {
        ...
        $.getJSON("../project/course/Controller/WxPayController.ashx?method=config&openId=" + params.oid + "&page=wmm_course.html&v=" + getQueryString("v"), function (data) {
            wx.config({
            ...
            wx.ready(function () {
                $.getJSON('../project/course/controller/CourseController.ashx?method=course', { courseId: params.cid }, function (result) {
                    var _shareData = {
                        title: result.name, // 分享标题
                        desc: result.intro,
                        groupTitle: result.name,
                        link: 'http://weixin.jiajiaozhihui.cn/app/pay/share_course.html?cid='+params.cid+'&ctype=2&path=http://weixin.jiajiaozhihui.cn/app/pay/wmm_course.html', // 分享链接
                        imgUrl: 'http://weixin.jiajiaozhihui.cn' + result.miniImgUrl, // 分享图标
                        type: 'link',
                        dataUrl: '',
                        success: function () { },
                        error: function () { }
