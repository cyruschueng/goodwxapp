...
namespace SfSoft.web.Read.Helper
{
    public class HomeProvide
    {
        ...
        {
            SfSoft.BLL.WX_Read_File bll = new SfSoft.BLL.WX_Read_File();
            string sql = "update WX_Read_File set Status=0 where Id=" + fileId;
        ...
        {
            SfSoft.BLL.WX_Read_Like bll = new SfSoft.BLL.WX_Read_Like();
            var lists = bll.GetModelList("AppId='" + appId + "' and  ReadFileId=" + readFileId.ToString()).AsEnumerable();
        ...
        private Models.Home.Player GetPlayer(string appId, string openId)
        {
            ...
            SfSoft.Model.WX_Read_User model = bll.GetModelList("AppId='" + appId + "' and OpenId='" + openId + "'").FirstOrDefault();
            if (model != null)
            {
        ...
        {
            string path = System.Web.HttpContext.Current.Server.MapPath("~/App/Project/Read/SysData/admin.json");
