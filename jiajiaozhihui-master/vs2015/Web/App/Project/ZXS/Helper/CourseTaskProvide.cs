...
{
    public class CourseTaskProvide
    {
        ...
        private static List<SfSoft.Model.WX_TestQuestion_Score> _gxdrList;

        private static string _appId;
        private static string _oppenId;
        private static int _themeId;
        private static int _week;
        private static int _questionActiveId = 0;
        ...
        public static IEnumerable<Models.ThemeTasks.TaskInfo> GetWeekTasks(int themeId, int week, IEnumerable<SfSoft.Model.WX_ZXS_Task> tasks, IEnumerable<SfSoft.Model.WX_ZXS_PlayerTask> userTasks, string appId, string openId)
        {
            ...
            {
                int.TryParse(m.Other, out _questionActiveId);
                var task = tasks.FirstOrDefault(e => e.Id == m.TaskId);
                TestCompleteNumber(_questionActiveId, task.TaskType ?? 0);
        ...
        public static SfSoft.Model.WX_ZXS_Theme GetThemeByThemeId(int themeId,string appId)
        {
            SfSoft.BLL.WX_ZXS_Theme bll = new SfSoft.BLL.WX_ZXS_Theme();
            var list= bll.GetModelList("Sn="+themeId+" and AppId='"+appId+"'");
            if (list.Count > 0) return list.FirstOrDefault<SfSoft.Model.WX_ZXS_Theme>();
            return new Model.WX_ZXS_Theme();
        }
        /// <summary>
        /// 单个任务
        /// </summary>
        ...
        public static IEnumerable<SfSoft.Model.WX_ZXS_PlayerTask> GetUserTasks(string appId, string openId, int themeId)
        {
            ...
            return bll.GetModelList("AppId='" + appId + "' and OpenId='" + openId + "' and ThemeId=" + themeId + " and Status=1").AsEnumerable();
        }
        /// <summary>
        /// 完成数量
        /// </summary>
        ...
        private static int CompleteNumber(int taskType, int taskId)
        {
            ...
            switch (taskType)
            {
                ...
                case 5:
                    ...
                    break;
                case 6:
                    number = _gxdrList.Where(e => e.QuestionActiveID == _questionActiveId).Count();
                    break;
                case 8:
                    number = TestScore();
                    break;
        ...
        private static bool IsTodayComplete(int taskId, int taskType)
        {
            ...
            switch (taskType)
            {
                ...
                case 8:
                    ...
                    break;
                case 6:
                    number = _gxdrList.Where(e => e.QuestionActiveID == _questionActiveId && e.CreateDate.Value.Date.Subtract(DateTime.Now.Date).Days == 0).Count();
                    break;
                case 5:
                    number = _signInList.Where(e => e.TaskId == taskId && e.CreateDate.Value.Date.Subtract(DateTime.Now.Date).Days == 0).Count();
                    break;
                default:
        ...
        private static List<SfSoft.Model.WX_TestQuestion_Score> TestCompleteNumber(int itemId, int taskType)
        {
            if (taskType == 6)
            {
                SfSoft.BLL.WX_TestQuestion_Score bll = new SfSoft.BLL.WX_TestQuestion_Score();
                _gxdrList = bll.GetModelList("QuestionActiveId=" + itemId + " and openId='" + _oppenId + "'");
                if (_gxdrList == null)
                {
                    return new List<SfSoft.Model.WX_TestQuestion_Score>();
                }
                return _gxdrList;
            }
            return new List<SfSoft.Model.WX_TestQuestion_Score>();
        ...
        {
            SfSoft.BLL.WX_TestQuestion_Item_Score bll = new SfSoft.BLL.WX_TestQuestion_Item_Score();
            SfSoft.Model.WX_TestQuestion_Item_Score model = bll.GetModel(_oppenId, _questionActiveId);
            if (model != null)
            {
        ...
        {
            return RedireUrl(courseId);
        }
        private  static string RedireUrl(int courseId)
        {
            ...
            if (model != null)
            {
                ...
                {
                    case 1:
                        url = ReturnUrl(courseId, "video");
                        break;
                    case 2:
                        url = ReturnUrl(courseId, "singleaudio");
                        break;
                    case 3:
                        url = ReturnUrl(courseId, "multipleaudio");
                        break;
                }
            }
            else {
        ...
        private static string ReturnUrl(int courseId, string page)
        {
            var state = "{\"appid\":\"app001\",\"hash\":\"empty\",\"kid\":\"" + courseId + "\",\"path\":\"" + page + "\"}";
            var url = App.Url.CourseUrl.ClientUrl + "app/views/redirect/default.html?state=" + System.Web.HttpUtility.UrlEncode(state);
            return url;
        }
    }
}