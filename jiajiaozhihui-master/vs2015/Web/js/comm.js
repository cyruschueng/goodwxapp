 ...
 * @param d_int   用户密码 
 * eg:  onsubmit="return isShortDateTime(form1.Email)"
 */
...
function permitDate(objDate,type)
{
		...
		switch(type){ 
				case 1:
					...
					valid = isShortDateTime(text);break; 
				case 2:
					strMsg="日期格式不对!\n标准格式如:2008-01-01";
...
/*
 * 功能：判断select是否选择
 * @param Obj  表单对象select，只适用于select
 * @param strCompValue 比较值  
 * @param sMsg   
 * 
 */
function isSelected(obj,strCompValue,sMsg)
{  
   var iSel = 0;
   iSel = obj.selectedIndex;
   if(obj.options[iSel].value==strCompValue){
     alert("["+sMsg+"]不能为空！");
     obj.focus();
	 return false;
   } else {
