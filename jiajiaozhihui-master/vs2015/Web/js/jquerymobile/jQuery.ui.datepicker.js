					...
					Datepicker.prototype,
					{
						...
						_updateDatepicker : function(inst) {
							...
							if (inst == $.datepicker._curInst
									...
									&& !inst.input.is(':disabled'))
								inst.input.focus();
							$(".ui-datepicker-prev", inst.dpDiv).buttonMarkup({
								iconpos : "notext",
								icon : "arrow-l",
								shadow : true,
								corners : true
							});
							$(".ui-datepicker-next", inst.dpDiv).buttonMarkup({
								iconpos : "notext",
								icon : "arrow-r",
								shadow : true,
								corners : true
							});
							$(".ui-datepicker-calendar a", inst.dpDiv)
									.buttonMarkup({
										corners : false,
										shadow : false
									});
							$(".ui-datepicker-calendar .ui-btn", inst.dpDivs)
									.each(
											function() {
												var el = $(this);
												// remove extra button markup -
												// necessary for date value to
												// be
												// interpreted correctly
												el.html(el.find(".ui-btn-text")
														.text());
											});
						},
						...
						_selectDate : function(id, dateStr) {
							...
							if (inst.inline) {
								this._updateDatepicker(inst);
								var onselectCallBack = this._get(inst,
										'onselectCallBack');
								var callBackId = this._get(inst,
								'callBackId');
								if (onselectCallBack)
									onselectCallBack
											.apply(this,[callBackId, dateStr, inst ]); // trigger
							} else {
								this._hideDatepicker();
						...
						_generateHTML : function(inst) {
							...
							var prev = (this._canAdjustMonth(inst, -1,
									drawYear, drawMonth) ? '<a class="ui-datepicker-prev ui-corner-all" href="#"  onclick="DP_jQuery_'
									+ dpuuid
									...
									+ '</span></a>'
									: (hideIfNoPrevNext ? ''
											: '<a class="ui-datepicker-prev ui-corner-all ui-state-disabled" href="#" title="'
													+ prevText
													+ '"><span class="ui-icon ui-icon-circle-triangle-'
													+ (isRTL ? 'e' : 'w')
													+ '">'
							...
							var next = (this._canAdjustMonth(inst, +1,
									drawYear, drawMonth) ? '<a class="ui-datepicker-next ui-corner-all" href="#" onclick="DP_jQuery_'
									+ dpuuid
									+ '.datepicker._adjustDate(\'#'
									+ inst.id
									+ '\', +'
									...
									+ '</span></a>'
									: (hideIfNoPrevNext ? ''
											: '<a class="ui-datepicker-next ui-corner-all ui-state-disabled" href="#" title="'
													+ nextText
													+ '"><span class="ui-icon ui-icon-circle-triangle-'
													+ (isRTL ? 'w' : 'e')
													+ '">'
							...
							for ( var row = 0; row < numMonths[0]; row++) {
								...
								for ( var col = 0; col < numMonths[1]; col++) {
									...
									calender += '<div class="ui-datepicker-header ui-widget-header ui-helper-clearfix ui-body-c ui-corner-top">'
											...
											+ '<tr>';
									var thead = (showWeek ? '<th class="ui-datepicker-week-col ui-bar-c">'
									...
									for ( var dow = 0; dow < 7; dow++) { // days
										...
										thead += '<th'
												+ ((dow + firstDay + 6) % 7 >= 5 ? ' class="ui-datepicker-week-end ui-bar-c"'
														: ' class="ui-bar-c"')
												+ '>' + '<span title="'
												+ dayNames[day] + '">'
												+ dayNamesMin[day]
									...
									for ( var dRow = 0; dRow < numRows; dRow++) { // create
										...
										calender += '<tr>';
										var tbody = (!showWeek ? ''
												: '<td class="ui-datepicker-week-col ui-body-c">'
														+ this
																._get(inst,
																		'calculateWeek')
																(printDate)
										...
										for ( var dow = 0; dow < 7; dow++) { // create
											...
											tbody += '<td class="ui-body-c '
													...
													(otherMonth
															...
															// other
															// months
															(unselectable ? '<a class="ui-state-default ui-disabled">'
																	+ printDate
																			.getDate()
																	+ '</a>'
																	: '<a class="ui-state-default'
																			+ (printDate
																					.getTime() == today
																					.getTime() ? ' ui-state-highlight ui-btn-up-e'
																					: '')
																			+ (printDate
																					.getTime() == currentDate
																					.getTime() ? ' ui-state-active ui-btn-active'
																					: '')
																			+ // highlight
																			// selected
																			// day
