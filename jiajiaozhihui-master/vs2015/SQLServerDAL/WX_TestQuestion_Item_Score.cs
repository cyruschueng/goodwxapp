using System;
using System.Data;
using System.Text;
using System.Data.SqlClient;
using SfSoft.IDAL;
using SfSoft.DBUtility;//Please add references
namespace SfSoft.SQLServerDAL
{
	/// <summary>
	/// 数据访问类:WX_TestQuestion_Item_Score
	/// </summary>
	public partial class WX_TestQuestion_Item_Score:IWX_TestQuestion_Item_Score
	{
		public WX_TestQuestion_Item_Score()
		{}
		#region  BasicMethod

		/// <summary>
		/// 得到最大ID
		/// </summary>
		public int GetMaxId()
		{
		return DbHelperSQL.GetMaxID("Item", "WX_TestQuestion_Item_Score"); 
		}

		/// <summary>
		/// 是否存在该记录
		/// </summary>
		public bool Exists(string OpenId,int Item)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) from WX_TestQuestion_Item_Score");
			strSql.Append(" where OpenId=@OpenId and Item=@Item ");
			SqlParameter[] parameters = {
					new SqlParameter("@OpenId", SqlDbType.NVarChar,100),
					new SqlParameter("@Item", SqlDbType.Int,4)			};
			parameters[0].Value = OpenId;
			parameters[1].Value = Item;

			return DbHelperSQL.Exists(strSql.ToString(),parameters);
		}


		/// <summary>
		/// 增加一条数据
		/// </summary>
		public bool Add(SfSoft.Model.WX_TestQuestion_Item_Score model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into WX_TestQuestion_Item_Score(");
			strSql.Append("OpenId,Item,Score,UsingTime)");
			strSql.Append(" values (");
			strSql.Append("@OpenId,@Item,@Score,@UsingTime)");
			SqlParameter[] parameters = {
					new SqlParameter("@OpenId", SqlDbType.NVarChar,100),
					new SqlParameter("@Item", SqlDbType.Int,4),
					new SqlParameter("@Score", SqlDbType.Int,4),
					new SqlParameter("@UsingTime", SqlDbType.Int,4)};
			parameters[0].Value = model.OpenId;
			parameters[1].Value = model.Item;
			parameters[2].Value = model.Score;
			parameters[3].Value = model.UsingTime;

			int rows=DbHelperSQL.ExecuteSql(strSql.ToString(),parameters);
			if (rows > 0)
			{
				return true;
			}
			else
			{
				return false;
			}
		}
		/// <summary>
		/// 更新一条数据
		/// </summary>
		public bool Update(SfSoft.Model.WX_TestQuestion_Item_Score model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update WX_TestQuestion_Item_Score set ");
			strSql.Append("Score=@Score,");
			strSql.Append("UsingTime=@UsingTime");
			strSql.Append(" where OpenId=@OpenId and Item=@Item ");
			SqlParameter[] parameters = {
					new SqlParameter("@Score", SqlDbType.Int,4),
					new SqlParameter("@UsingTime", SqlDbType.Int,4),
					new SqlParameter("@OpenId", SqlDbType.NVarChar,100),
					new SqlParameter("@Item", SqlDbType.Int,4)};
			parameters[0].Value = model.Score;
			parameters[1].Value = model.UsingTime;
			parameters[2].Value = model.OpenId;
			parameters[3].Value = model.Item;

			int rows=DbHelperSQL.ExecuteSql(strSql.ToString(),parameters);
			if (rows > 0)
			{
				return true;
			}
			else
			{
				return false;
			}
		}

		/// <summary>
		/// 删除一条数据
		/// </summary>
		public bool Delete(string OpenId,int Item)
		{
			
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from WX_TestQuestion_Item_Score ");
			strSql.Append(" where OpenId=@OpenId and Item=@Item ");
			SqlParameter[] parameters = {
					new SqlParameter("@OpenId", SqlDbType.NVarChar,100),
					new SqlParameter("@Item", SqlDbType.Int,4)			};
			parameters[0].Value = OpenId;
			parameters[1].Value = Item;

			int rows=DbHelperSQL.ExecuteSql(strSql.ToString(),parameters);
			if (rows > 0)
			{
				return true;
			}
			else
			{
				return false;
			}
		}


		/// <summary>
		/// 得到一个对象实体
		/// </summary>
		public SfSoft.Model.WX_TestQuestion_Item_Score GetModel(string OpenId,int Item)
		{
			
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 OpenId,Item,Score,UsingTime from WX_TestQuestion_Item_Score ");
			strSql.Append(" where OpenId=@OpenId and Item=@Item ");
			SqlParameter[] parameters = {
					new SqlParameter("@OpenId", SqlDbType.NVarChar,100),
					new SqlParameter("@Item", SqlDbType.Int,4)			};
			parameters[0].Value = OpenId;
			parameters[1].Value = Item;

			SfSoft.Model.WX_TestQuestion_Item_Score model=new SfSoft.Model.WX_TestQuestion_Item_Score();
			DataSet ds=DbHelperSQL.Query(strSql.ToString(),parameters);
			if(ds.Tables[0].Rows.Count>0)
			{
				return DataRowToModel(ds.Tables[0].Rows[0]);
			}
			else
			{
				return null;
			}
		}


		/// <summary>
		/// 得到一个对象实体
		/// </summary>
		public SfSoft.Model.WX_TestQuestion_Item_Score DataRowToModel(DataRow row)
		{
			SfSoft.Model.WX_TestQuestion_Item_Score model=new SfSoft.Model.WX_TestQuestion_Item_Score();
			if (row != null)
			{
				if(row["OpenId"]!=null)
				{
					model.OpenId=row["OpenId"].ToString();
				}
				if(row["Item"]!=null && row["Item"].ToString()!="")
				{
					model.Item=int.Parse(row["Item"].ToString());
				}
				if(row["Score"]!=null && row["Score"].ToString()!="")
				{
					model.Score=int.Parse(row["Score"].ToString());
				}
				if(row["UsingTime"]!=null && row["UsingTime"].ToString()!="")
				{
					model.UsingTime=int.Parse(row["UsingTime"].ToString());
				}
			}
			return model;
		}

		/// <summary>
		/// 获得数据列表
		/// </summary>
		public DataSet GetList(string strWhere)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select OpenId,Item,Score,UsingTime ");
			strSql.Append(" FROM WX_TestQuestion_Item_Score ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
			}
			return DbHelperSQL.Query(strSql.ToString());
		}

		/// <summary>
		/// 获得前几行数据
		/// </summary>
		public DataSet GetList(int Top,string strWhere,string filedOrder)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select ");
			if(Top>0)
			{
				strSql.Append(" top "+Top.ToString());
			}
			strSql.Append(" OpenId,Item,Score,UsingTime ");
			strSql.Append(" FROM WX_TestQuestion_Item_Score ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
			}
			strSql.Append(" order by " + filedOrder);
			return DbHelperSQL.Query(strSql.ToString());
		}

		/// <summary>
		/// 获取记录总数
		/// </summary>
		public int GetRecordCount(string strWhere)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) FROM WX_TestQuestion_Item_Score ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
			}
			object obj = DbHelperSQL.GetSingle(strSql.ToString());
			if (obj == null)
			{
				return 0;
			}
			else
			{
				return Convert.ToInt32(obj);
			}
		}
		/// <summary>
		/// 分页获取数据列表
		/// </summary>
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("SELECT * FROM ( ");
			strSql.Append(" SELECT ROW_NUMBER() OVER (");
			if (!string.IsNullOrEmpty(orderby.Trim()))
			{
				strSql.Append("order by T." + orderby );
			}
			else
			{
				strSql.Append("order by T.Item desc");
			}
			strSql.Append(")AS Row, T.*  from WX_TestQuestion_Item_Score T ");
			if (!string.IsNullOrEmpty(strWhere.Trim()))
			{
				strSql.Append(" WHERE " + strWhere);
			}
			strSql.Append(" ) TT");
			strSql.AppendFormat(" WHERE TT.Row between {0} and {1}", startIndex, endIndex);
			return DbHelperSQL.Query(strSql.ToString());
		}

		/*
		/// <summary>
		/// 分页获取数据列表
		/// </summary>
		public DataSet GetList(int PageSize,int PageIndex,string strWhere)
		{
			SqlParameter[] parameters = {
					new SqlParameter("@tblName", SqlDbType.VarChar, 255),
					new SqlParameter("@fldName", SqlDbType.VarChar, 255),
					new SqlParameter("@PageSize", SqlDbType.Int),
					new SqlParameter("@PageIndex", SqlDbType.Int),
					new SqlParameter("@IsReCount", SqlDbType.Bit),
					new SqlParameter("@OrderType", SqlDbType.Bit),
					new SqlParameter("@strWhere", SqlDbType.VarChar,1000),
					};
			parameters[0].Value = "WX_TestQuestion_Item_Score";
			parameters[1].Value = "Item";
			parameters[2].Value = PageSize;
			parameters[3].Value = PageIndex;
			parameters[4].Value = 0;
			parameters[5].Value = 0;
			parameters[6].Value = strWhere;	
			return DbHelperSQL.RunProcedure("UP_GetRecordByPage",parameters,"ds");
		}*/

		#endregion  BasicMethod
		#region  ExtensionMethod
        public DataSet GetList(string openid, string province, string city, string activityid, int top)
        {
            SqlParameter[] parameters = {
					new SqlParameter("@openid", SqlDbType.NVarChar, 100),
					new SqlParameter("@province", SqlDbType.NVarChar, 100),
					new SqlParameter("@city", SqlDbType.NVarChar,100),
					new SqlParameter("@activityid", SqlDbType.NVarChar,10),
					new SqlParameter("@top", SqlDbType.Int,4)
					};
            parameters[0].Value = openid;
            parameters[1].Value = province;
            parameters[2].Value = city;
            parameters[3].Value = activityid;
            parameters[4].Value = top;
            return DbHelperSQL.RunProcedure("pro_QuestionRank", parameters, "ds");
        }
		#endregion  ExtensionMethod
	}
}

