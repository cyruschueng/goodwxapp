...
//var yshurl = "http://api2.yshfresh.com/api/ysh/";
//var yshurl = "http://10.2.15.59:6001/api/ysh/";
...
function renderMain(tpl, data) {
 ...
 $(document).ready(function() {
        ...
        getDataList(yshurl+'getMenu',{},function(d){
...
var getsesskeying=false;
// 获取数据方法
// 获取数据方法
var getDataList = function(url, param, fcs, is_check) {
	  console.log(url);
    console.log(getsesskeying);
	// if(getsesskeying) return;

	showYshLoding()
	// if (url.indexOf('getSesskey') == -1) {
	//     getsesskeying=true;
	...
	$.ajax({
		...
		success: function(ret) {
			...
			if (ret.state != 0) {
				if(ret.state==401 || ret.state==402){		//sesskey失效，重新获取
					getsesskeying=true;					
					setCookie("sesskey", '');
					getDataList(yshurl+'getSesskey',{}, function(d){
						if (d.sesskey) {
							sesskey = d.sesskey;
							setCookie("sesskey", sesskey);
							getDataList(url, param, fcs, is_check);
		...
		error: function(e) {
			closeYshLoding()
			alert("APi 错误:"+JSON.stringify(e));
		}
	});
};

function err_msg(msg,e){
	var s;
	if(typeof(e)=='object'){
		s=JSON.stringify(e);
	}else{
...
// 获取数据方法
//ysh_getDataList = function(url, param, fcs, is_check) {
//    showLoding();
//    $.ajax({
//        type: "post",
//        url: url,
...
userInfo = function(funs) {
	var url = yshurl + "/api/get/userinfo"
	var user = ""
	var param = {
...
// 没有数据时候的显示
noData = function(view, error) {
	if (!error)
		error = "没有获取到数据"

	var childList = view.childNodes;
	var str = "<div style='text-align: center;'><img style='width: 50%;padding-top:10%' src= img/jingya.png><p style='font-color:red;display:black;margin:0 auto;font-family: '微软雅黑';'>" + error + "</p><div>"
