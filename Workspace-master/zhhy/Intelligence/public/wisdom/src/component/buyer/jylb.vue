<template>
	<div class="page-jylb">
		<div v-infinite-scroll="loadMore" infinite-scroll-disabled="loading" infinite-scroll-distance="1" style="padding-bottom: 2rem;">
			<div class="dealList">
				<ul>
					<li v-for="item in goodList" @click="tospxqPage(item.ProductId)">
						<div class="left fl">
      				...
      				commom.getDataList('ysh/GetOrderList',{
      					...
      					if(d.aaData && d.aaData.length > 0){
      						for(var i = 0, l = d.aaData.length; i < l; i++){
      							if(d.aaData[i].orderdate==null){
				    				d.aaData[i].orderdate="暂无"
				    			}
				    			vm.goodList.push(d.aaData[i])
      						}
				    		vm.loading = false
		            		vm.pageIndex++
      					} else {
