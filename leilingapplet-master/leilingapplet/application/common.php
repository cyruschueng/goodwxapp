...
// +----------------------------------------------------------------------
use app\index\model\UserErrorLog;
// 应用公共文件
class pinyin{  
        private $d=array(  
           array("a",-20319),  
        ...
        private function c($str){  
            ...
            for($i=0;$i<strlen($str);$i++){  
                ...
                if(preg_match("/^[0-9a-zA-Z]$/",strtoupper(substr($this->g($p),0,1)))){
                    $ret.=strtoupper(substr($this->g($p),0,1)); 
                } 
            }  
            return $ret;  
        }  
    ...
    function get_pinyin($str,$charset="utf-8"){ 
        $pinyin=new pinyin();  
        return strtoupper($pinyin->get_pinyin($str,$charset));  
    } 
     ...
     * @param $userid 用户id
     */
    function do_log($action,$telephone,$detail,$userid)
    {
      $usererrorlog = New UserErrorLog();
      $ua = $_SERVER['HTTP_USER_AGENT'];
      $ipaddr = get_client_ip(0, true);
      $usererrorlog->action = $action;
      $usererrorlog->telephone = $telephone;
      $usererrorlog->detail = $detail;
      $usererrorlog->ip = $ipaddr;
      $usererrorlog->createtime = date('Y-m-d H:i:s');
      $usererrorlog->userid = $userid;
      $usererrorlog->useragent = $ua;
      $usererrorlog->save();
    }

    function send_msg_aliyun($tel, $msg){
    
      include VENDOR_PATH.'sms/aliyun-php-sdk-core/Config.php';
