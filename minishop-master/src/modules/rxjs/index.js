...
var _root = {
  ...
  // 其他：setImmediate, clearImmediate, process MessageChannel document postMessage onmessage importScripts addEventListener MessageChannel
}
...
var Observable = (function () {
   ...
   * be left uncaught.
   *
   * The second way is to give up on Observer object altogether and simply provide callback functions in place of its
   * methods. This means you can provide three functions as arguments to `subscribe`, where first function is
   * equivalent of a `next` method, second of an `error` method and third of a `complete` method. Just as in case of
   * Observer, if you do not need to listen for something, you can omit a function, preferably by passing `undefined`
   * or `null`, since `subscribe` recognizes these functions by where they were placed in function call. When it comes
   * to `error` function, just as before, if not provided, errors emitted by an Observable will be thrown.
   *
   * Whatever style of calling `subscribe` you use, in both cases it returns a Subscription object.
   * This object allows you to call `unsubscribe` on it, which in turn will stop work that an Observable does and will
   * clean up all resources that an Observable used. Note that cancelling a subscription will not call `complete`
   * callback provided to `subscribe` function, which is reserved for a regular completion signal that comes from an
   * Observable.
   *
   ...
   *  Observable.
   * @param {Function} error (optional) A handler for a terminal event resulting from an error. If no error handler is
   *   provided, the error will be thrown as unhandled.
 ...
 *
 * @example <caption>Spawn a new interval Observable for each click event, and blend their outputs as one
 *   Observable</caption> var clicks = Rx.Observable.fromEvent(document, 'click'); var higherOrder = clicks.map((ev) =>
 *   Rx.Observable.interval(1000)); var firstOrder = higherOrder.mergeAll(); firstOrder.subscribe(x => console.log(x));
 *
 ...
 *
 * Returned Observable emits the same notifications (nexted values, complete and error events) as the source
 * Observable,
 ...
 *
 * As a matter of fact, `observeOn` accepts second parameter, which specifies in milliseconds with what delay
 * notifications will be emitted. The main difference between {@link delay} operator and `observeOn` is that
 * `observeOn` will delay all notifications - including error notifications - while `delay` will pass through error
...
/* tslint:enable:max-line-length */
/**
 * When any of the provided Observable emits an complete or error notification, it immediately subscribes to the next
 * one that was passed.
 *
 ...
 * as the source.
 *
 * `onErrorResumeNext` returns an Observable that starts by subscribing and re-emitting values from the source
 * Observable. When its stream of values ends - no matter if Observable completed or emitted an error -
 * `onErrorResumeNext` will subscribe to the first Observable that was passed as an argument to the method. It will
 * start re-emitting its values as well and - again - when that stream ends, `onErrorResumeNext` will proceed to
 * subscribing yet another Observable in provided series, no matter if previous Observable completed or ended with an
 * error. This will be happening until there is no more Observables left in the series, at which point returned
 * Observable will complete - even if the last subscribed stream ended with an error.
 *
 ...
 * previous items.
 *
 * If a keySelector function is provided, then it will project each value from the source observable into a new value
 * that it will check for equality with previously projected values. If a keySelector function is not provided, it will
 * use each value from the source observable directly with an equality check against previous values.
 *
 * In JavaScript runtimes that support `Set`, this operator will use a `Set` to improve performance of the distinct
 * value checking.
 *
 * In other runtimes, this operator will use a minimal implementation of `Set` that relies on an `Array` and `indexOf`
 * under the hood, so performance will degrade as more values are checked for distinction. Even in newer browsers, a
 * long-running `distinct` use might result in memory leaks. To help alleviate this in some scenarios, an optional
 * `flushes` parameter is also provided so that the internal `Set` can be "flushed", basically clearing it of values.
 *
 ...
 * @param {function} [compare] Optional comparison function called to test if an item is distinct from the previous
 *   item in the source.
 * @return {Observable} An Observable that emits items from the source Observable with distinct values based on the key
 *   specified.
 ...
 * @return {Observable} An Observable of booleans that determines if all items of the source Observable meet the
 *   condition specified.
 * @method every
 * @owner Observable
 */
 ...
 * it passes each source value through a transformation function to get corresponding output values.</span>
 *
...
/**
 * The Max operator operates on an Observable that emits numbers (or items that can be compared with a provided
 * function), and when source Observable completes it emits a single item: the item with the largest value.
 *
...
/**
 * The Min operator operates on an Observable that emits numbers (or items that can be compared with a provided
 * function), and when source Observable completes it emits a single item: the item with the smallest value.
 *
 ...
 *
 * @example <caption>On every click (starting from the second), emit the relative distance to the previous
 *   click</caption> var clicks = Rx.Observable.fromEvent(document, 'click'); var pairs = clicks.pairwise(); var
 *   distance = pairs.map(pair => { var x0 = pair[0].clientX; var y0 = pair[0].clientY; var x1 = pair[1].clientX; var
 *   y1 = pair[1].clientY; return Math.sqrt(Math.pow(x0 - x1, 2) + Math.pow(y0 - y1, 2));
 * });
...
function publish (selector) {
  return selector ? multicast.call(this, function () { return new Subject(); }, selector) : multicast.call(this, new Subject());
}
 ...
 *
 * @return {Observable<T>} An Observable that upon connection causes the source Observable to emit items to its
 *   Observers.
 * @method share
 * @owner Observable
 */
 ...
 *
 * @param {Observable} notifier - The second Observable that has to emit an item before the source Observable's
...
var TestScheduler = (function (_super) {
  ...
  TestScheduler.parseMarbles = function (marbles, values, errorValue, materializeInnerObservables) {
    ...
    var getValue = typeof values !== 'object' ? function (x) { return x; } : function (x) {
      // Support Observable-of-Observables
