<template>
	<div class="editor">
		<el-form :model="editForm" :rules="rules" ref="editForm" label-width="100px" class="demo-ruleForm">
			...
			<el-form-item>
				<mavon-editor v-model="editForm.detail" @change="getHtml('value', 'render')"/>
			</el-form-item>
			<el-form-item>
...
<script>
	...
	export default{
		...
		methods:{
			...
			//取得头像上传签名
			get_sign(){
				this.axios.get(`${this.url}/dtk/news/pic/sign`).then(res=>{
					if (res.data.code!=='OK') {
			...
			add(){
				this.axios.post(`${this.url}/dtk/news`,{
			...
			edit(){
				this.axios.put(`${this.url}/dtk/news/${this.$route.query.id}`,{
					title:this.editForm.title,
					brief:this.editForm.brief,
					detail:this.editForm.detail,
					pic:this.editForm.pic
			...
			getnews(){
				this.axios.get(`${this.url}/dtk/news/${this.$route.query.id}`).then(res=>{
					if (res.data.code==='OK') {
