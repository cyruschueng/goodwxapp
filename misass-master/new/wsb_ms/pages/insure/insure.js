...
Page({
  ...
  data: {
    ...
    _cargoType: 0,                              //货物类型状态
    cargoType: '货物类型',
    cargoOneArr: ['易碎品', '非易碎品'],            //货物类型名单
    cargoTwoArr: ['普通货物', '贵重货物'],            //货物类型名单
    //保险产品名称
    _product:0,
    product:'保险产品名称',
  ...
  //跳转到投保人
  jumpPolicymsg:function(){
    wx.navigateTo({
      url: '../policymsg/policymsg',
    })
  },
   //跳转到被投保人
  jumpAssuredmsg: function () {
    wx.navigateTo({
  ...
  rightDate: function (e) {
    if (this.data.dateValue == '起运时间'){
      this.setData({
        dateValue: date.getFullYear() + '年1月1日'
      })
    }
    this.setData({
      _date: 0
  ...
  cargoChange: function (e) {
    const val = e.detail.value;
    this.setData({
      cargoType: this.data.cargoOneArr[val[0]] + this.data.cargoTwoArr[val[1]]
    })
  },
  //确定 
