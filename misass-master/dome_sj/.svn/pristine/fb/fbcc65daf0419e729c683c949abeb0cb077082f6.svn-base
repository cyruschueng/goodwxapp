...
function isPoneNum(pone) {
    ...
    if (!myreg.test(pone)) {
        $('.alert_ds').css('display','block');
        $('#trueBack').click(function () {
            $('.alert_ds').css('display','none');
        });
        return false;
    } else {
        // console.log(myreg.test(pone));
        return true;
    }
}
...
$('.address_edit').click(function () {
    let index = $(this).attr('value');   //index 为值
    $('#maskDol').show();
    $('#mask').removeClass('mask_hide');
    $('.detail_address,.rec_name,.rec_tel,rec_address').keyup(function () {   //输入时候 判断是否存在空值
        if($('.rec_name').val()&&$('.rec_tel').val()&&$('.rec_address').val()&&$('.detail_address').val()){
            $('#address_save').addClass('address_save')
        }else{
            $('#address_save').removeClass('address_save')
        }
        if($('#address_save').hasClass('address_save')){    //全部有值时候 点击按钮变色
            $('#address_save').click(function () {         //输入地址  点击保存
