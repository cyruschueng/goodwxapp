...
Page({
  ...
  data: {
    ...
    _product:0,
    product:'保险产品名称',
    productArr: ['物流险','运输险','交通基本险'],
    ifDown:true,
    _goods:0,
    goods:'货物价值',
    goodsArr:['10万','20万','30万','40万','50万','100万'],
    _packType: 0,                              //包装类型状态
    packType: '包装类型',
  ...
  bindChange: function (e) {
    ...
    this.setData({
      ...
      dateValue: this.data.years[val[0]] + '年' + this.data.months[val[1]] + '月' + this.data.days[val[2]] + '日' + this.data.hours[val[3]] + '时' + this.data.minute[val[4]] + '分' + this.data.second[val[5]] + '秒'
    })
  },
  //取消
  ...
  rightDate: function (e) {
    if (this.data.dateValue == '起运时间'){
      this.setData({
      dateValue: date.getFullYear() + '年2月2日1时1分1秒'  
      })
    }
    this.setData({
      _date: 0
  ...
  productChange: function (e) {
    const val = e.detail.value;
    this.setData({
      product: this.data.productArr[val[0]]
    })
  },
  //确定
  rightProduct: function (e) {
    if (this.data.product == '保险产品名称') {
      this.setData({
        product: this.data.productArr[0]
      })
    }
    this.setData({
      _product: 0
...
/***********************包装类型*******************************/
  ...
  goodsChange: function (e) {
    const val = e.detail.value;
    console.log(this.data.goodsArr[val[0]])
    this.setData({
      goods: this.data.goodsArr[val[0]]
    })
  },
  //确定
  rightGoods: function (e) {
    if (this.data.goods == '货物价值') {
      this.setData({
        goods: this.data.goodsArr[0]
      })
    }
    this.setData({
      _goods: 0,
  ...
  previewPolicy:function(e){
    if (this.data._iconChange){
      wx.navigateTo({
        url: '../policyview/policyview',
      })
    }
  }
