...
function setTotal() {
    ...
    $('.car_li>.icon-checked').siblings('.car_list_detail').each(function(i,obj){
        var num = parseInt($('.car_li>.icon-checked').siblings('.car_list_detail').find('.car_detail_price>.car_detail_pri>p:nth-of-type(2)')[i].textContent);
        var price = parseFloat($('.car_li>.icon-checked').siblings('.car_list_detail').find('.car_detail_price>.car_detail_num>span')[i].textContent);
        totalNum += num*price;
    })
    $("#car_total").html(totalNum);
}
