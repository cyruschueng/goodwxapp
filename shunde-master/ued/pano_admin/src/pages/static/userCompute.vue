      ...
      <div>
        ...
        <div class="center">
          <div class="inputBox" style="width: 200px">
            <el-input class="search-input-4" :placeholder="header.search.placeholder" @change="searchTable" suffix-icon="el-icon-search"
...
<script>
  import panoChart from '../../components/business/userChart.vue'
  import panoForm from '../../components/business/userForm.vue'
  export default {
    name: 'pano',
    data () {
      return {
    ...
    methods: {
      ...
      selectSingle () {
        this.$api.getUserCountForm({
          userId: this.panoData.userId,
          ...
          for (var k = 0; k < this.x_content.length; k++) {
            if (res.data[this.x_content[k]]) {
              this.list[0].data[k] = (res.data[this.x_content[k]].deployStat)
              this.list[1].data[k] = (res.data[this.x_content[k]].loginStat)
              this.list[2].data[k] = (res.data[this.x_content[k]].viewStat)
            }
          }
        })
      },
      tableChoose (value) {
        this.panoData.userId = value.userId
        this.selectSingle()
      },
      dropChanged (val) {
        this.panoData.fromDate = ''
