        ...
        <div class="center">
          <div class="inputBox" style="width: 200px;">
            <el-input class="search-input-4" :placeholder="header.search.placeholder" @change="searchTable" suffix-icon="el-icon-search"
                      size="small" style="padding-left:10px; height: 23px" v-model="data.gridCode"/>
          </div>
        </div>
      </div>
    </div>
...
<script>
  ...
  import panoForm from '../../components/business/gridForm.vue'
  export default {
    name: 'pano',
    data () {
      return {
    ...
    methods: {
      ...
      selectSingle () {
        this.$api.getGridCountForm({
          gridCode: this.panoData.gridCode,
      ...
      tableChoose (value) {
        this.panoData.gridCode = value.gridCode
        this.selectSingle()
      },
      dropChanged (val) {
