<template>
  <div id="roamContainer">
    <common-container :selected="selected" bgColor="#fff" :open-shadow="isShadow" :gridCode="gridCode"
      ...
      <div id="new-roam">
        <pano-bar id="barContainer" :panoId="panoId" :panoName="panoname" :gridCode="gridCode" :isEmpty="isEmpty"
                  :leftCode="leftCode"
                  :rightCode="rightCode" @pbclick="pbc" :markerList="markerList"
        ...
        <pano-view :play-flag="playFlag" :initPanoEnable="initFlag" :vr-flag="vrFlag" :isEmpty="isEmpty"
                   :panoId="panoId" :propPanoName="panoname" @northclick="northClick"
                   @westclick="westClick" @eastclick="eastClick" @southclick="southClick"
                   @currentLabelNextPoiClick="currentNextPoi"
  ...
  export default {
    ...
    mounted () {
      if (this.$route.params && this.$route.params.gridCode) {
        ...
        this.panoname = this.$route.params.name
    ...
    methods: {
      currentNextPoi () {
        ...
        if (this.currentPoiIndex > this.array.length - 1) {
          this.currentPoiIndex = 0
        }
        if (this.firstPanoId === this.panoId) {
          this.markerList = this.copy(this.mainMarkerList)
        }
        this.panoId = this.array[this.currentPoiIndex].panoInfo.panoId
        this.panoname = this.array[this.currentPoiIndex].panoInfo.name
        this.initFlag = false
        let self = this
        setTimeout(function () {
          self.initFlag = true
      ...
      watchPanoView (index) {
        ...
        this.panoname = this.array[index].panoInfo.name
        ...
        if (this.firstPanoId === this.panoId) {
          ...
          this.markerList = [{name: '下一张', directionType: 'M'}]
        }
        setTimeout(function () {
          self.initFlag = true
        })
        ...
        this.$api.getPoiList({
          ...
          if (res.code === 0) {
            if (res.data && res.data.length > 0) {
              ...
              this.panoname = res.data[0].panoInfo.name
