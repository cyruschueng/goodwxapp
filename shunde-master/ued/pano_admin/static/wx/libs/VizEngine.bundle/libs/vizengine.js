(function(){
    ...
    var mergeObject = com.vizengine.mergeObject = function() {
        var output = {};
    ...
    com.vizengine.Module = function(path) {
        this.path = path;
        this.script = null;
    ...
    var Module = com.vizengine.Module;
    ...
    Module.prototype.isTextTemplate = function() {
        return isEndsWith(this.path,".txt");
    }
    ...
    Module.prototype.processTextContent = function(callback) {
        this.packageObject = this.script;
        this.loadState = 3;
        callback();
    }
    ...
    Module.prototype.processTextTemplate = function(callback) {
        ...
        while((res = searchContent.match(reg)) != null) {
            includes.push(com.vizengine.view.View.resolvePath(this.path,res[1]));
            searchContent = searchContent.substring(res.index + res[0].length);
        }

        var finishCallback = function() {
            for(var i = 0 ; i < includes.length; i++) {
                if(!com.vizengine.moduleManager.isModuleProcessed(includes[i])){
                    return;
                }
            }
            for(var i = 0 ; i < includes.length; i++) {
                var module = com.vizengine.moduleManager[includes[i]];
                if(module.isJson()) {
        ...
        for(var i = 0 ; i < includes.length; i++) {
            com.vizengine.moduleManager.loadModule(includes[i],function(){
                finishCallback();
            });
        }
    }
    Module.prototype.processViewTemplateContent = function(callback) {
        ...
        this.packageObject = function(){
            ...
            var view = com.vizengine.view.View.parse(output,{contextPath:self.path},data,self.path);
        ...
        var loadNextModule = function(){
            if(currentLoadIndex < includes.length) {
                com.vizengine.moduleManager.loadModule(includes[currentLoadIndex],function(){
                    currentLoadIndex++;
                    loadNextModule();
                });
                return;
    ...
    Module.prototype.processModelContent = function(callback) {
        ...
        com.vizengine.moduleManager.contextPath = this.path;
        if(this.isScript()) {
            ...
            this.processTextContent(onContentProceessed);
        }
    }
    Module.prototype.load = function(callback) {
        ...
        loadResource(this.path,function(script){
            if(self.isJson() || self.isViewTemplate()) {
                script = com.vizengine.view.View.deleteComment(script);
            }
            self.script = script;
    ...
    com.vizengine.moduleManager = new ModuleManager();
    ...
    window.module = function() {
        ...
        for(var i = 0 ; i < libs.length ; i++) {
            libs[i] = com.vizengine.moduleManager.resolveScriptPath(libs[i]);
        }
        ...
        var checkRequiredLibsProcessed = function(){
            for(var i = 0 ; i < libs.length; i++) {
                if(!com.vizengine.moduleManager.isModuleProcessed(libs[i])) {
                    return false;
                }
            }
            return true;
        ...
        var getPackageObjectsArg = function() {
            var args = [];
            for(var i = 0 ; i < libs.length; i++) {
                args.push(com.vizengine.moduleManager[libs[i]].packageObject);
            }
            return args;
        }
        ...
        var loadNextModule = function() {
            if(currentLoadIndex < libs.length){
                com.vizengine.moduleManager.loadModule(libs[currentLoadIndex],function(){
                    currentLoadIndex++;
                    loadNextModule();
                });
                return;
    ...
    var customTemplate = function() {
        ...
        template.get = function(filename) {
            var path = com.vizengine.view.View.resolvePath(Module.currentTemplateContextPath,filename);
    ...
    module([
        "/libs/vizengine_model3d.js"
    ],function(){
        customTemplate();
        if(window.onVizEngineLoaded != null) {
            window.onVizEngineLoaded();
        }
    });
})();
