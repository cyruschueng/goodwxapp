...
Page({
  ...
  connect() {
    var that = this;
    that.pushMessage(createSystemMessage('欢迎加入交理聊聊...'));
    //记录进入房间
    welcome(that);
    ...
    BmobSocketIo.onInitListen = function () {
      ...
      that.tunnel = true;
      that.pushMessage(createSystemMessage('成功加入交理聊聊...'));
    ...
    BmobSocketIo.onUpdateTable = function (tablename, data) {
      ...
      if (tablename == "welcome") {
        if (data.online != 1) {
          that.pushMessage(createSystemMessage(`${data.nickName}已加入交理聊聊。`));
        } else {
          // that.pushMessage(createSystemMessage(`${data.nickName}已退出交理聊聊。`));
        }
      }
    };
    if (!this.pageReady) {
  ...
  sendMessage(e) {
    // 信道当前不可用
    if (!this.tunnel) {
      this.pushMessage(createSystemMessage('您还没有加入交理聊聊，请稍后重试'));
      return;
    }
