...
Page({
  ...
  returnMoneyEvent: function (e) {
    ...
    if (arrPrice[1] != null && arrPrice[1].length > 2) {
      ...
      var rgpoint = new RegExp('^\\d{1,4}(\\.\\d{0,2})?$', 'g');//判断用户输入的是否为小数点
    ...
    if (getApp().globalData.return_num>0){
      ...
      if (that.touchEndTime - that.touchStartTime < 350) {
        ...
        if (currentTime - lastTapTime < 300) {
          ...
          if (100 * getApp().globalData.myCount >= 100 * getApp().globalData.returnMoney && getApp().globalData.returnMoney >= 1.01 && getApp().globalData.returnMoney <= 500) {
            setTimeout(function () {
              wx.showModal({
                title: '提现确认',
                content: '提现不得小于1.01元，含手续费1%',
                success: function (res) {
                  if (res.confirm) {
                    wx.request({
                      url: 'https://www.mqtp8.cn/applet/getmoney/getMoney', //余额接口
                      method: 'GET',
                      header: {
          ...
          else if (getApp().globalData.returnMoney < 1.01) {
            wx.showToast({
              title: '提现不小于1.01元',
              icon: 'loading',
              duration: 1000,
  ...
  onLoad: function () { 
    ...
    //请求余额接口数据
    wx.request({ 
      url: 'https://www.mqtp8.cn/applet/pay/mycount', //余额接口
      method: 'POST',
      header: {
        'Content-Type': 'application/json'
      },
