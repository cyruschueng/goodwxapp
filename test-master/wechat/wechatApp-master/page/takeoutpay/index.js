...
Page({
  ...
  onLoad: function (options) {
    ...
    var shopId = decodeURIComponent(options.shopId)
    var memberId = decodeURIComponent(options.memberId)
    var bizId = decodeURIComponent(options.bizId)
    var deskId = decodeURIComponent(options.deskId)
    ...
    console.log('进来参数')
    console.log(shopId, memberId, bizId)
  },
  onShow: function () {
  ...
  cart: function (meunId, aaa, price, name2, menuPicture){
    ...
    if (aaa) {//改份数
      ...
      subdata.num = 1;
      subdata.memberId = that.data.memberId;
      subdata.shopId = that.data.shopId;
  ...
  settle: function(){
    ...
    wx.navigateTo({
      url: './cart/index?shopId=' + 6 + '&bizId=' + 6 + '&memberId=' + 37 + '&deskId=' + 250
    })
  },
