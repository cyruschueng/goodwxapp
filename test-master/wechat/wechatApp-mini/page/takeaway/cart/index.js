...
Page({
  ...
  settle_b(that, formId){
    ...
    var leastMoney=  wx.getStorageSync('takeoutData').leastMoney

    if (leastMoney > this.data.price) {
      console.log(leastMoney)
      wx.showToast({
        image: '/image/i/x.png',
        title: '满' + leastMoney + '元起送！',
      })
      return
    }
    ...
    wx.getStorage({
      ...
      success: (res) => {
        ...
        app.commonAjax('/miniapp/cat/takeout/submit', ['appid', 'shopId', 'bizId', 'memberId','shopName'], subdata,  (res) => {
          ...
          if(res.data.code == 0){
            ...
            wx.redirectTo({
              url: "/page/takeaway/submit/index?orderId=" + res.data.data.orderId
            })
            ...
            //     // wx.redirectTo({
            //     //   url: "/page/takeaway/submit/index?orderId=" + res.data.data.orderMember.id 
            //     // })
