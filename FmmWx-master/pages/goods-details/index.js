...
Page({
  ...
  onLoad: function (e) {
    ...
    var goodDetail = Bmob.Object.extend("goodDetail");
    var query = new Bmob.Query(goodDetail);
    query.equalTo("contentId", e.contentId);
    query.first({
      success: function (result) {
        console.log(result);
        console.log(result.get("basicInfo"));
        ...
        if (result.properties) {
          for (var i = 0; i < result.properties.length;i++){
            selectSizeTemp = selectSizeTemp + " " + result.properties[i].name;
          }
          that.setData({
            hasMoreSelect:true,
            selectSize:that.data.selectSize + selectSizeTemp,
            selectSizePrice: result.basicInfo.minPrice,
          });
        }
        that.setData({
          goodsDetail: result,
          selectSizePrice: result.get("basicInfo").minPrice,
          buyNumMax: result.get("basicInfo").stores,
          buyNumber: (result.get("basicInfo").stores>0) ? 1: 0
        });
